...
static void strip_leading_nops(zend_op_array *op_array, zend_basic_block *b)
{
	...
	while (b->len > 0 && opcodes[b->start].opcode == ZEND_NOP)
	{
		...
		if (b->len == 2 && (op_array->opcodes[b->start + 1].opcode == ZEND_JMPZ || op_array->opcodes[b->start + 1].opcode == ZEND_JMPNZ) && (op_array->opcodes[b->start + 1].op1_type & (IS_CV | IS_CONST)) && b->start > 0 && zend_is_smart_branch(op_array->opcodes + b->start - 1))
		{
			break;
		}
		b->start++;
...
static void strip_nops(zend_op_array *op_array, zend_basic_block *b)
{
	...
	while (i < b->start + b->len)
	{
		...
		if (i + 1 < b->start + b->len && (op_array->opcodes[i + 1].opcode == ZEND_JMPZ || op_array->opcodes[i + 1].opcode == ZEND_JMPNZ) && op_array->opcodes[i + 1].op1_type & (IS_CV | IS_CONST) && zend_is_smart_branch(op_array->opcodes + j - 1))
		{
...
static int get_const_switch_target(zend_cfg *cfg, zend_op_array *op_array, zend_basic_block *block, zend_op *opline, zval *val)
{
	...
	zval *zv;
	if ((opline->opcode == ZEND_SWITCH_LONG && Z_TYPE_P(val) != IS_LONG) || (opline->opcode == ZEND_SWITCH_STRING && Z_TYPE_P(val) != IS_STRING))
	{
		...
		*/
		...
		case ZEND_IS_NOT_EQUAL:
				 ...
				 Z_TYPE(ZEND_OP1_LITERAL(opline)) == IS_TRUE))
			{
				 ...
				 */
				opline->opcode =
					((opline->opcode != ZEND_IS_NOT_EQUAL) == ((Z_TYPE(ZEND_OP1_LITERAL(opline))) == IS_TRUE)) ? ZEND_BOOL : ZEND_BOOL_NOT;
				COPY_NODE(opline->op1, opline->op2);
					  ...
					  Z_TYPE(ZEND_OP2_LITERAL(opline)) == IS_TRUE))
			{
				 ...
				 */
				opline->opcode =
					((opline->opcode != ZEND_IS_NOT_EQUAL) == ((Z_TYPE(ZEND_OP2_LITERAL(opline))) == IS_TRUE)) ? ZEND_BOOL : ZEND_BOOL_NOT;
				SET_UNUSED(opline->op2);
				goto optimize_bool;
			}
			break;
		...
		case ZEND_EXIT:
			if (opline->op1_type & (IS_TMP_VAR | IS_VAR))
			{
				...
				if (src && src->opcode == ZEND_QM_ASSIGN)
				{
					...
					while (op < opline)
					{
						if ((op->op1_type == opline->op1_type && op->op1.var == opline->op1.var) || (op->op2_type == opline->op1_type && op->op2.var == opline->op1.var))
						{
							optimize = 0;
							break;
						}
