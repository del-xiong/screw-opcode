...
int zend_optimizer_get_persistent_constant(zend_string *name, zval *result, int copy) {
    ...
    if (c) {
        if ((ZEND_CONSTANT_FLAGS(c) & CONST_PERSISTENT) && !(ZEND_CONSTANT_FLAGS(c) & CONST_DEPRECATED) &&
...
static int
get_const_switch_target(zend_cfg *cfg, zend_op_array *op_array, zend_basic_block *block, zend_op *opline, zval *val) {
...
static void zend_optimize_block(zend_basic_block *block, zend_op_array *op_array, zend_bitset used_ext, zend_cfg *cfg,
                                zend_op **Tsource, uint32_t *opt_count) {
    zend_op *opline, *src;
        ...
        switch (opline->opcode) {
                ...
                if (opline->op1_type & (IS_TMP_VAR | IS_VAR)) {
                    ...
                    if (last_op == opline - 1) {
                        ...
                        if (!Z_REFCOUNTED(ZEND_OP1_LITERAL(last_op))) {
                            ...
                            Z_STR(ZEND_OP1_LITERAL(last_op)) = zend_string_extend(Z_STR(ZEND_OP1_LITERAL(last_op)), l,
                                                                                  0);
                        }
                        Z_TYPE_INFO(ZEND_OP1_LITERAL(last_op)) = IS_STRING_EX;
                        memcpy(Z_STRVAL(ZEND_OP1_LITERAL(last_op)) + old_len, Z_STRVAL(ZEND_OP1_LITERAL(opline)),
                               Z_STRLEN(ZEND_OP1_LITERAL(opline)));
                        Z_STRVAL(ZEND_OP1_LITERAL(last_op))
                        [l] = '\0';
            ...
            case ZEND_FAST_CONCAT:
                ...
                if (opline->op2_type == IS_CONST &&
                    ...
                    if (src &&
                        ...
                        memcpy(Z_STRVAL(ZEND_OP2_LITERAL(src)) + old_len, Z_STRVAL(ZEND_OP2_LITERAL(opline)),
                               Z_STRLEN(ZEND_OP2_LITERAL(opline)));
                        Z_STRVAL(ZEND_OP2_LITERAL(src))
                        [l] = '\0';
            ...
            case ZEND_BW_XOR:
                if (opline->op1_type == IS_CONST &&
                    ...
                    if (zend_optimizer_eval_binary_op(&result, opline->opcode, &ZEND_OP1_LITERAL(opline),
                                                      &ZEND_OP2_LITERAL(opline)) == SUCCESS) {
                        literal_dtor(&ZEND_OP1_LITERAL(opline));
                        literal_dtor(&ZEND_OP2_LITERAL(opline));
                        opline->opcode = ZEND_QM_ASSIGN;
                        SET_UNUSED(opline->op2);
            ...
            case ZEND_CAST:
                if (opline->op1_type == IS_CONST) {
                    ...
                    if (zend_optimizer_eval_cast(&result, opline->extended_value, &ZEND_OP1_LITERAL(opline)) ==
                        SUCCESS) {
                        literal_dtor(&ZEND_OP1_LITERAL(opline));
                        opline->opcode = ZEND_QM_ASSIGN;
                        opline->extended_value = 0;
            ...
            case ZEND_EXIT:
                if (opline->op1_type == IS_TMP_VAR) {
                    ...
                    if (src && src->opcode == ZEND_QM_ASSIGN) {
                        ...
                        while (op < opline) {
                            if ((op->op1_type == opline->op1_type && op->op1.var == opline->op1.var) ||
                                (op->op2_type == opline->op1_type && op->op2.var == opline->op1.var)) {
                                optimize = 0;
                                break;
                            }
                            op++;
    ...
    for (b = blocks; b < end; b++) {
        ...
        switch (opline->opcode) {
            ...
            case ZEND_MATCH: {
                ...
                zval *zv;
                uint32_t s = 0;
                ZEND_ASSERT(b->successors_count ==
                            (opline->opcode == ZEND_MATCH ? 1 : 2) + zend_hash_num_elements(jumptable));
...
static zend_always_inline zend_basic_block
*

get_target_block(const zend_cfg *cfg, zend_basic_block *block, int n, uint32_t *opt_count) {
    int b;
...
get_follow_block(const zend_cfg *cfg, zend_basic_block *block, int n, uint32_t *opt_count) {
    int b;
    zend_basic_block * target_block = cfg->blocks + block->successors[n];
...
get_next_block(const zend_cfg *cfg, zend_basic_block *block) {
...
static void
zend_jmp_optimization(zend_basic_block *block, zend_op_array *op_array, const zend_cfg *cfg, int *jmp_hitlist,
                      uint32_t *opt_count) {
