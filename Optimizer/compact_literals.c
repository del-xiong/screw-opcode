            ...
            switch (Z_TYPE(op_array->literals[i])) {
                ...
                case IS_LONG:
                    if (LITERAL_NUM_RELATED(info[i].flags) == 1) {
                        ...
                        key = zend_string_init(Z_STRVAL(op_array->literals[i + 1]), Z_STRLEN(op_array->literals[i + 1]),
                                               0);
                    ...
                    break;
                case IS_DOUBLE:
                    if ((pos = zend_hash_str_find(&double_hash, (char *) &Z_DVAL(op_array->literals[i]),
                                                  sizeof(double))) != NULL) {
                        map[i] = Z_LVAL_P(pos);
                    } else {
                        map[i] = j;
                        ZVAL_LONG(&zv, j);
                        zend_hash_str_add_new(&double_hash, (char *) &Z_DVAL(op_array->literals[i]), sizeof(double),
                                              &zv);
                        if (i != j) {
                            op_array->literals[j] = op_array->literals[i];
                            info[j] = info[i];
            ...
            switch (opline->opcode) {
                ...
                case ZEND_POST_DEC_STATIC_PROP:
                    if (opline->op1_type == IS_CONST) {
                        ...
                        if (class_slot[opline->op2.constant] >= 0) {
                            opline->extended_value =
                                    class_slot[opline->op2.constant] | (opline->extended_value & ZEND_FETCH_OBJ_FLAGS);
                        } else {
                            opline->extended_value = cache_size | (opline->extended_value & ZEND_FETCH_OBJ_FLAGS);
                            class_slot[opline->op2.constant] = cache_size;
                            cache_size += sizeof(void *);
