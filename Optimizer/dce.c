...
is_bad_mod(const zend_ssa *ssa, int use, int def) {
    if (def < 0) {
...
{
if (
is_bad_mod(ssa, ssa_op
->op1_use, ssa_op->op1_def)) {
return 1;
}
...
is_bad_mod(ssa, ssa_op
->op1_use, ssa_op->op1_def)
|| ssa->vars[ssa_op->op1_def].escape_state !=
...
add_operands_to_worklists(context *ctx, zend_op *opline, zend_ssa_op *ssa_op, zend_ssa *ssa, int check) {
...
static zend_bool dce_instr(context *ctx, zend_op *opline, zend_ssa_op *ssa_op) {
    ...
    if (opline->opcode == ZEND_FREE
        && (ssa->var_info[ssa_op->op1_use].type &
    ...
    if ((opline->op1_type & (IS_VAR | IS_TMP_VAR)) && !is_var_dead(ctx, ssa_op->op1_use)) {
        if (!try_remove_var_def(ctx, ssa_op->op1_use, ssa_op->op1_use_chain, opline)) {
            if (ssa->var_info[ssa_op->op1_use].type &
                (MAY_BE_STRING | MAY_BE_ARRAY | MAY_BE_OBJECT | MAY_BE_RESOURCE | MAY_BE_REF)
    ...
    if ((opline->op2_type & (IS_VAR | IS_TMP_VAR)) && !is_var_dead(ctx, ssa_op->op2_use)) {
        if (!try_remove_var_def(ctx, ssa_op->op2_use, ssa_op->op2_use_chain, opline)) {
            if (ssa->var_info[ssa_op->op2_use].type &
                (MAY_BE_STRING | MAY_BE_ARRAY | MAY_BE_OBJECT | MAY_BE_RESOURCE | MAY_BE_REF)) {
                if (free_var >= 0) {
...
may_break_varargs(const zend_op_array *op_array, const zend_ssa *ssa, const zend_ssa_op *ssa_op) {
    if (ssa_op->op1_def >= 0
        ...
        while (i > block->start) {
            ...
            if (zend_bitset_in(ctx.instr_worklist, i)) {
                ...
                if (op_data >= 0) {
                    ...
                    may_have_side_effects(op_array, ssa, &op_array->opcodes[i], &ssa->ops[i], ctx.reorder_dtor_effects)
