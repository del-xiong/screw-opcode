...
static void zend_ssa_remove_nops(zend_op_array *op_array, zend_ssa *ssa, zend_optimizer_ctx *ctx) {
    ...
    shiftlist = (uint32_t * )
    do_alloca(sizeof(uint32_t) * op_array->last, use_heap);
...
can_elide_return_type_check(
        zend_op_array
*op_array,
zend_ssa *ssa, zend_ssa_op
*ssa_op) {
...
if (
ZEND_TYPE_FULL_MASK(info
->type) & (MAY_BE_CALLABLE|MAY_BE_ITERABLE|MAY_BE_STATIC)) {
return 0;
}

if (
ZEND_TYPE_HAS_CLASS(info
->type)) {
if (!use_info->ce || !def_info->ce || !
safe_instanceof(use_info
->ce, def_info->ce)) {
return 0;
}
}
...
take_successor_ex(zend_ssa *ssa, int block_num, zend_basic_block *block, int target_block) {
    int i;
                    ...
                    if (opline->op1_type == IS_CONST) {
                        ...
                        if (block->successors[0] == next_block_num) {
                            ...
                            if (opline->op1_type == IS_CV && (OP1_INFO() & MAY_BE_UNDEF)) {
                                ...
                                opline->op2.num = 0;
                            } else if (opline->op1_type == IS_CV || !(OP1_INFO() &
                                                                      (MAY_BE_STRING | MAY_BE_ARRAY | MAY_BE_OBJECT |
                        ...
                        if (block->successors[0] == block->successors[1]) {
                            ...
                            if (block->successors[0] == next_block_num) {
                                if (opline->op1_type == IS_CV && (OP1_INFO() & MAY_BE_UNDEF)) {
                                    opline->opcode = ZEND_CHECK_VAR;
                                    opline->op2.num = 0;
                                } else if (opline->op1_type == IS_CV || !(OP1_INFO() & (MAY_BE_STRING | MAY_BE_ARRAY |
                                                                                        MAY_BE_OBJECT |
                                                                                        MAY_BE_RESOURCE |
                                                                                        MAY_BE_REF))) {
                                    zend_ssa_remove_instr(ssa, opline, ssa_op);
                            ...
                            } else if ((opline->op1_type == IS_CV && !(OP1_INFO() & MAY_BE_UNDEF)) || !(OP1_INFO() &
                                                                                                        (MAY_BE_STRING |
                                                                                                         MAY_BE_ARRAY |
                                                                                                         MAY_BE_OBJECT |
                                                                                                         MAY_BE_RESOURCE |
...
void
zend_dfa_optimize_op_array(zend_op_array *op_array, zend_optimizer_ctx *ctx, zend_ssa *ssa, zend_call_info **call_map) {
...
// op_1: QM_ASSIGN #src_var.T #orig_var.CV [undef,scalar] -> #v.CV,    NOP
                    ...
                    if (orig_var < 0 || zend_ssa_unlink_use_chain(ssa, op_1, orig_var)) {
                        ...
                        if (op_array->opcodes[op_2].opcode == ZEND_SUB
                            ...
                            && !(ssa->var_info[ssa->ops[op_2].op1_use].type &
                                 (MAY_BE_FALSE | MAY_BE_TRUE | MAY_BE_STRING | MAY_BE_ARRAY | MAY_BE_OBJECT |
                                  MAY_BE_RESOURCE | MAY_BE_REF))) {
                            ...
                            ssa->ops[op_2].op1_def = v;
                                   ...
                                   && !(ssa->var_info[ssa->ops[op_2].op2_use].type &
                                        (MAY_BE_FALSE | MAY_BE_TRUE | MAY_BE_STRING | MAY_BE_ARRAY | MAY_BE_OBJECT |
                                         MAY_BE_RESOURCE | MAY_BE_REF))) {
                        ...
                        if (zend_ssa_unlink_use_chain(ssa, op_1, orig_var)) {
                            ...
                            if (op_array->opcodes[op_2].opcode == ZEND_SUB
                                ...
                                ssa->ops[op_2].op1_def = v;
                                       ...
                                       && op_array->opcodes[op_2].op1.var == op_array->opcodes[op_2].result.var
                                       && op_array->opcodes[op_2].op2_type == IS_CONST
                                       && Z_TYPE_P(CT_CONSTANT_EX(op_array, op_array->opcodes[op_2].op2.constant)) ==
                                          IS_LONG
                                       && Z_LVAL_P(CT_CONSTANT_EX(op_array, op_array->opcodes[op_2].op2.constant)) == 1
                                       && ssa->ops[op_2].op1_use >= 0
                                       && !(ssa->var_info[ssa->ops[op_2].op1_use].type &
...
// op_1: ASSIGN #orig_var.CV [undef,scalar] -> #v.CV, CONST|TMPVAR => QM_ASSIGN v.CV, CONST|TMPVAR
                       ...
                       && ssa->ops[op_1].op1_use >= 0
                       && !(ssa->var_info[ssa->ops[op_1].op1_use].type &
                            (MAY_BE_FALSE | MAY_BE_TRUE | MAY_BE_STRING | MAY_BE_ARRAY | MAY_BE_OBJECT |
                             MAY_BE_RESOURCE | MAY_BE_REF))) {
