        ...
        switch (opline->opcode) {
            ...
            case ZEND_CASE_STRICT:
                if (opline->op1_type == IS_CONST &&
                    ...
                    if (zend_optimizer_eval_binary_op(&result, opline->opcode, &ZEND_OP1_LITERAL(opline),
                        ...
                        if (zend_optimizer_replace_by_const(op_array, opline + 1, IS_TMP_VAR, opline->result.var,
                                                            &result)) {
                            MAKE_NOP(opline);
                        } else {
                ...
                if (opline->op2_type == IS_CONST) {
                    if (opline->extended_value == ZEND_ADD
                        ...
                        if (Z_TYPE(ZEND_OP2_LITERAL(opline)) == IS_STRING) {
                            ...
                            if (is_numeric_string(Z_STRVAL(ZEND_OP2_LITERAL(opline)),
                                                  Z_STRLEN(ZEND_OP2_LITERAL(opline)), NULL, NULL, 0)) {
                        ...
                        if (Z_TYPE(ZEND_OP2_LITERAL(opline)) != IS_LONG) {
                            /* don't optimise if it should produce a runtime numeric string error */
                            if (!(Z_TYPE(ZEND_OP2_LITERAL(opline)) == IS_STRING
                                  && !is_numeric_string(Z_STRVAL(ZEND_OP2_LITERAL(opline)),
                                                        Z_STRLEN(ZEND_OP2_LITERAL(opline)), NULL, NULL, 0))) {
            ...
            case ZEND_CAST:
                if (opline->op1_type == IS_CONST) {
                    ...
                    if (zend_optimizer_eval_cast(&result, opline->extended_value, &ZEND_OP1_LITERAL(opline)) ==
                        SUCCESS) {
                        literal_dtor(&ZEND_OP1_LITERAL(opline));
                        if (zend_optimizer_replace_by_const(op_array, opline + 1, opline->result_type,
                                                            opline->result.var, &result)) {
                            MAKE_NOP(opline);
                        } else {
                            opline->opcode = ZEND_QM_ASSIGN;
            ...
            case ZEND_FETCH_CONSTANT:
                if (opline->op2_type == IS_CONST &&
                    ...
                    memcmp(Z_STRVAL(ZEND_OP2_LITERAL(opline)), "__COMPILER_HALT_OFFSET__",
                ...
                if (opline->op2_type == IS_CONST &&
                    ...
                    if (!zend_optimizer_get_persistent_constant(Z_STR(ZEND_OP2_LITERAL(opline)), &c, 1)) {
                        if (!ctx->constants ||
                            !zend_optimizer_get_collected_constant(ctx->constants, &ZEND_OP2_LITERAL(opline), &c)) {
                            break;
                        }
                    }
                    ...
                    if (opline->op1_type == IS_CONST &&
                        ...
                        if (op_array->scope &&
                            ...
                            if ((ce = zend_hash_find_ptr(EG(class_table),
                                                         Z_STR(op_array->literals[opline->op1.constant + 1]))) ==
                                NULL ||
            ...
            case ZEND_DO_ICALL: {
                ...
                if (Z_STRLEN(ZEND_OP2_LITERAL(init_opline)) == sizeof("define") - 1 &&
                    zend_binary_strcasecmp(Z_STRVAL(ZEND_OP2_LITERAL(init_opline)),
                                           Z_STRLEN(ZEND_OP2_LITERAL(init_opline)), "define", sizeof("define") - 1) ==
                    0) {
                    ...
                    if (Z_TYPE(ZEND_OP1_LITERAL(send1_opline)) == IS_STRING &&
                        ...
                        if (collect_constants) {
                            zend_optimizer_collect_constant(ctx, &ZEND_OP1_LITERAL(send1_opline),
                                                            &ZEND_OP1_LITERAL(send2_opline));
                        }

                        if (RESULT_UNUSED(opline) &&
                            !zend_memnstr(Z_STRVAL(ZEND_OP1_LITERAL(send1_opline)), "::", sizeof("::") - 1,
                                          Z_STRVAL(ZEND_OP1_LITERAL(send1_opline)) +
                                          Z_STRLEN(ZEND_OP1_LITERAL(send1_opline)))) {
                ...
                if (!send2_opline &&
                    ...
                    if ((Z_STRLEN(ZEND_OP2_LITERAL(init_opline)) == sizeof("function_exists") - 1 &&
                        ...
                        /* dirname(IS_CONST/IS_STRING) -> IS_CONST/IS_STRING */
                               ...
                               IS_ABSOLUTE_PATH(Z_STRVAL(ZEND_OP1_LITERAL(send1_opline)),
                                                Z_STRLEN(ZEND_OP1_LITERAL(send1_opline)))) {
                        zend_string *dirname = zend_string_init(Z_STRVAL(ZEND_OP1_LITERAL(send1_opline)),
                                                                Z_STRLEN(ZEND_OP1_LITERAL(send1_opline)), 0);
            ...
            case ZEND_DECLARE_CONST:
                ...
                break;
#if 0
                /* see ext/screwopcode/tests/bug78961.phpt */
        //		case ZEND_FETCH_R:
                case ZEND_FETCH_W:
        //		case ZEND_FETCH_RW:
                case ZEND_FETCH_IS:
