...
void zend_optimizer_pass1(zend_op_array *op_array, zend_optimizer_ctx *ctx)
{
	...
	zend_bool collect_constants = (ZEND_OPTIMIZER_PASS_15 & ctx->optimization_level) ? (op_array == &ctx->script->main_op_array) : 0;
			...
			*/
				...
				Z_TYPE(ZEND_OP1_LITERAL(send1_opline)) == IS_STRING)
			{
					 ...
					 !zend_optimizer_is_disabled_func("is_callable", sizeof("is_callable") - 1)))
				{
					...
					if ((func = zend_hash_find_ptr(EG(function_table), lc_name)) != NULL && func->type == ZEND_INTERNAL_FUNCTION && func->module->type == MODULE_PERSISTENT
#ifdef ZEND_WIN32
