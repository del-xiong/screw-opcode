...
static void zend_dump_type_constraint(const zend_op_array *op_array, const zend_ssa *ssa,
                                      const zend_ssa_type_constraint *constraint, uint32_t dump_flags) {
    fprintf(stderr, " TYPE");
    zend_dump_type_info(constraint->type_mask, constraint->ce, 1, dump_flags);
}

static void
zend_dump_range_constraint(const zend_op_array *op_array, const zend_ssa *ssa, const zend_ssa_range_constraint *r,
                           uint32_t dump_flags) {
...
void zend_dump_op(const zend_op_array *op_array, const zend_basic_block *b, const zend_op *opline, uint32_t dump_flags,
    ...
    if (ZEND_VM_EXT_JMP_ADDR == (flags & ZEND_VM_EXT_MASK)) {
        if (b) {
            ...
            fprintf(stderr, " %04u", (uint32_t)
            ZEND_OFFSET_TO_OPLINE_NUM(op_array, opline, opline->extended_value));
        }
    }
            ...
            if (ssa_op) {
                ...
                if (ssa_var_num >= 0) {
                    ...
                    zend_dump_ssa_var(op_array, ssa, ssa_var_num, opline->result_type,
                                      EX_VAR_TO_NUM(opline->result.var), dump_flags);
                }
            } else {
                fprintf(stderr, " ");
                zend_dump_var(op_array, opline->result_type, EX_VAR_TO_NUM(opline->result.var));
...
static void
zend_dump_op_line(const zend_op_array *op_array, const zend_basic_block *b, const zend_op *opline, uint32_t dump_flags,
                  const void *data) {
    int len = 0;
...
static void zend_dump_block_header(const zend_cfg *cfg, const zend_op_array *op_array, const zend_ssa *ssa, int n,
                                   uint32_t dump_flags) {
...
//TODO: this is useful only for JIT???
    ...
    if (func_info) {
        ...
        zend_dump_type_info(func_info->return_info.type, func_info->return_info.ce,
                            func_info->return_info.is_instanceof, dump_flags);
