...
get_ssa_var_info(const zend_ssa *ssa, int ssa_var_num) {
    if (ssa->var_info && ssa_var_num >= 0) {
        return ssa->var_info[ssa_var_num].type;
    } else {
        return MAY_BE_UNDEF | MAY_BE_RC1 | MAY_BE_RCN | MAY_BE_REF | MAY_BE_INDIRECT | MAY_BE_ANY |
...
zend_add_will_overflow(zend_long
a,
zend_long b
) {
...
static zend_always_inline zend_bool
zend_sub_will_overflow(zend_long
a,
zend_long b
) {
...
uint32_t zend_array_element_type(uint32_t
t1,
zend_uchar op_type,
int write,
int insert
