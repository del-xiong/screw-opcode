...
int zend_optimizer_eval_unary_op(zval *result, zend_uchar opcode, zval *op1) /* {{{ */
{
    ...
    if (unary_op) {
        if (opcode == ZEND_BW_NOT && Z_TYPE_P(op1) != IS_LONG && Z_TYPE_P(op1) != IS_DOUBLE &&
...
alloc_cache_slots(zend_op_array
*op_array,
uint32_t num
)
{
...
int zend_optimizer_update_op1_const(zend_op_array *op_array,
    ...
    switch (opline->opcode) {
        ...
        case ZEND_POST_DEC_STATIC_PROP:
            ...
            opline->op1.constant = zend_optimizer_add_literal(op_array, val);
            if (opline->op2_type == IS_CONST &&
                (opline->extended_value & ~ZEND_FETCH_OBJ_FLAGS) + sizeof(void *) == op_array->cache_size) {
                op_array->cache_size += sizeof(void *);
            } else {
                opline->extended_value =
                        alloc_cache_slots(op_array, 3) | (opline->extended_value & ZEND_FETCH_OBJ_FLAGS);
            }
            break;
        case ZEND_SEND_VAR:
        ...
        case ZEND_ECHO: {
            ...
            /* (e.g. for ext/screwopcode/tests/opt/sccp_010.phpt) */
            break;
        }
        case ZEND_CONCAT:
    ...
    switch (opline->opcode) {
        ...
        case ZEND_ASSIGN_STATIC_PROP_OP:
            ...
            if (opline->op1_type != IS_CONST) {
                opline->extended_value = alloc_cache_slots(op_array, 1) | (opline->extended_value &
                                                                           (ZEND_RETURNS_FUNCTION | ZEND_ISEMPTY |
                    ...
                    while (opline < end) {
                        if (opline->op1_type == type && opline->op1.var == var) {
                            if (
                                    ...
                                    opline->opcode == ZEND_SWITCH_LONG || opline->opcode == ZEND_SWITCH_STRING ||
...
void zend_optimizer_migrate_jump(zend_op_array *op_array, zend_op *new_opline, zend_op *opline) {
    switch (new_opline->opcode) {
        ...
        case ZEND_FAST_CALL:
            ...
            break;
        case ZEND_JMPZNZ:
            new_opline->extended_value = ZEND_OPLINE_NUM_TO_OFFSET(op_array, new_opline,
                                                                   ZEND_OFFSET_TO_OPLINE_NUM(op_array, opline,
                                                                                             opline->extended_value));
            /* break missing intentionally */
        case ZEND_JMPZ:
        case ZEND_JMPNZ:
        ...
        case ZEND_MATCH: {
            ...
            {
                Z_LVAL_P(zv) = ZEND_OPLINE_NUM_TO_OFFSET(op_array, new_opline,
                                                         ZEND_OFFSET_TO_OPLINE_NUM(op_array, opline, Z_LVAL_P(zv)));
            }
...
void zend_optimizer_shift_jump(zend_op_array *op_array, zend_op *opline, uint32_t *shiftlist) {
    switch (opline->opcode) {
        case ZEND_JMP:
        case ZEND_FAST_CALL:
            ZEND_SET_OP_JMP_ADDR(opline, opline->op1,
                                 ZEND_OP1_JMP_ADDR(opline) - shiftlist[ZEND_OP1_JMP_ADDR(opline) - op_array->opcodes]);
            break;
        case ZEND_JMPZNZ:
            opline->extended_value = ZEND_OPLINE_NUM_TO_OFFSET(op_array, opline,
                                                               ZEND_OFFSET_TO_OPLINE_NUM(op_array, opline,
                                                                                         opline->extended_value) -
                                                               shiftlist[ZEND_OFFSET_TO_OPLINE_NUM(op_array, opline,
                                                                                                   opline->extended_value)]);
            /* break missing intentionally */
        case ZEND_JMPZ:
        case ZEND_JMPNZ:
        ...
        case ZEND_ASSERT_CHECK:
        case ZEND_JMP_NULL:
            ZEND_SET_OP_JMP_ADDR(opline, opline->op2,
                                 ZEND_OP2_JMP_ADDR(opline) - shiftlist[ZEND_OP2_JMP_ADDR(opline) - op_array->opcodes]);
            break;
        case ZEND_CATCH:
            if (!(opline->extended_value & ZEND_LAST_CATCH)) {
                ZEND_SET_OP_JMP_ADDR(opline, opline->op2, ZEND_OP2_JMP_ADDR(opline) -
                                                          shiftlist[ZEND_OP2_JMP_ADDR(opline) - op_array->opcodes]);
            }
            break;
        case ZEND_FE_FETCH_R:
        case ZEND_FE_FETCH_RW:
        ...
        case ZEND_MATCH: {
            ...
            ZEND_HASH_FOREACH_VAL(jumptable, zv)
            {
                Z_LVAL_P(zv) = ZEND_OPLINE_NUM_TO_OFFSET(op_array, opline,
                                                         ZEND_OFFSET_TO_OPLINE_NUM(op_array, opline, Z_LVAL_P(zv)) -
                                                         shiftlist[ZEND_OFFSET_TO_OPLINE_NUM(op_array, opline,
                                                                                             Z_LVAL_P(zv))]);
            }
    ...
    switch (opline->opcode) {
        ...
        case ZEND_INIT_METHOD_CALL:
            if (opline->op1_type == IS_UNUSED && opline->op2_type == IS_CONST &&
                Z_TYPE_P(CRT_CONSTANT(opline->op2)) == IS_STRING && op_array->scope &&
                !(op_array->fn_flags & ZEND_ACC_TRAIT_CLONE) && !(op_array->scope->ce_flags & ZEND_ACC_TRAIT)) {
...
static void zend_redo_pass_two(zend_op_array *op_array) {
    ...
    if (op_array->last_literal) {
        ...
        op_array->literals = (zval * )(
                ((char *) op_array->opcodes) + ZEND_MM_ALIGNED_SIZE_EX(sizeof(zend_op) * op_array->last, 16));
    } else {
        if (op_array->literals) {
            efree(op_array->literals);
        }
        ...
        switch (opline->opcode) {
            ...
            case ZEND_ARRAY_KEY_EXISTS:
                if (opline->result_type & IS_TMP_VAR) {
                    ...
                    if (opline + 1 < end) {
                        if ((opline + 1)->opcode == ZEND_JMPZ && (opline + 1)->op1_type == IS_TMP_VAR &&
                            (opline + 1)->op1.var == opline->result.var) {
                            opline->result_type = IS_SMART_BRANCH_JMPZ | IS_TMP_VAR;
                        } else if ((opline + 1)->opcode == ZEND_JMPNZ && (opline + 1)->op1_type == IS_TMP_VAR &&
                                   (opline + 1)->op1.var == opline->result.var) {
...
static void zend_redo_pass_two_ex(zend_op_array *op_array, zend_ssa *ssa) {
    ...
    while (opline < end) {
        ...
        uint32_t op1_info = opline->op1_type == IS_UNUSED ? 0 : (OP1_INFO() & (MAY_BE_UNDEF | MAY_BE_ANY | MAY_BE_REF |
                                                                               MAY_BE_ARRAY_OF_ANY |
                                                                               MAY_BE_ARRAY_KEY_ANY));
        uint32_t op2_info = opline->op1_type == IS_UNUSED ? 0 : (OP2_INFO() & (MAY_BE_UNDEF | MAY_BE_ANY | MAY_BE_REF |
                                                                               MAY_BE_ARRAY_OF_ANY |
                                                                               MAY_BE_ARRAY_KEY_ANY));
        uint32_t res_info =
                ...
                ? ((ssa->ops[opline - op_array->opcodes].op1_def >= 0) ? (OP1_DEF_INFO() &
                                                                          (MAY_BE_UNDEF | MAY_BE_ANY | MAY_BE_REF |
                                                                           MAY_BE_ARRAY_OF_ANY | MAY_BE_ARRAY_KEY_ANY))
                                                                       : MAY_BE_ANY)
                : (opline->result_type == IS_UNUSED ? 0 : (RES_INFO() & (MAY_BE_UNDEF | MAY_BE_ANY | MAY_BE_REF |
                                                                         MAY_BE_ARRAY_OF_ANY | MAY_BE_ARRAY_KEY_ANY)));
