...
static zend_bool
needs_pi(const zend_op_array *op_array, zend_dfg *dfg, zend_ssa *ssa, int from, int to, int var) /* {{{ */
{
        ...
        int from, int to, int var) /* {{{ */
{
    ...
    phi->use_chains = (zend_ssa_phi **) (((char *) phi->sources) +
                                         ZEND_MM_ALIGNED_SIZE(sizeof(int) * ssa->cfg.blocks[to].predecessors_count));
...
static int find_adjusted_tmp_var(const zend_op_array *op_array, uint32_t build_flags, zend_op *opline, uint32_t var_num,
...
_zend_ssa_rename_op(const zend_op_array *op_array, const zend_op *opline, uint32_t k, uint32_t build_flags,
                    int ssa_vars_count, zend_ssa_op *ssa_ops, int *var) /* {{{ */
{
    const zend_op *next;
...
int zend_ssa_rename_op(const zend_op_array *op_array, const zend_op *opline, uint32_t k, uint32_t build_flags,
                       int ssa_vars_count, zend_ssa_op *ssa_ops, int *var) /* {{{ */
{
...
static int
zend_ssa_rename(const zend_op_array *op_array, uint32_t build_flags, zend_ssa *ssa, int *var, int n) /* {{{ */
{
    zend_basic_block *blocks = ssa->cfg.blocks;
...
int zend_build_ssa(zend_arena **arena, const zend_script *script, const zend_op_array *op_array, uint32_t build_flags,
                   zend_ssa *ssa) /* {{{ */
{
    zend_basic_block *blocks = ssa->cfg.blocks;
    ...
    for (j = 0; j < blocks_count; j++) {
        ...
        if (!zend_bitset_empty(phi + j * set_size, set_size)) {
            ZEND_BITSET_REVERSE_FOREACH(phi + j * set_size, set_size, i)
            {
                ...
                phi->use_chains = (zend_ssa_phi **) (((char *) phi->sources) + ZEND_MM_ALIGNED_SIZE(
                        sizeof(int) * ssa->cfg.blocks[j].predecessors_count));
...
static inline zend_ssa_phi
**
zend_ssa_next_use_phi_ptr(zend_ssa
*ssa,
int var, zend_ssa_phi
*p) /* {{{ */
{
if (p->pi >= 0) {
...
static inline void
zend_ssa_remove_use_of_phi_source(zend_ssa *ssa, zend_ssa_phi *phi, int source, zend_ssa_phi *next_use_phi) /* {{{ */
{
...
static inline void
zend_ssa_remove_phi_source(zend_ssa *ssa, zend_ssa_phi *phi, int pred_offset, int predecessors_count) /* {{{ */
{
    ...
    if (pred_offset < predecessors_count) {
        memmove(phi->sources + pred_offset, phi->sources + pred_offset + 1,
                (predecessors_count - pred_offset) * sizeof(uint32_t));
        memmove(phi->use_chains + pred_offset, phi->use_chains + pred_offset + 1,
                (predecessors_count - pred_offset) * sizeof(zend_ssa_phi *));
    }
