...
#if defined(COMPILE_DL_SCREWOPCODE)
...
static void (*accelerator_orig_zend_error_cb)(int type, const char *error_filename, const uint32_t error_lineno,
                                              zend_string *message);
...
void zend_accel_schedule_restart_if_necessary(zend_accel_restart_reason reason) {
    if ((((double) ZSMMG(wasted_shared_memory)) / ZCG(accel_directives).memory_consumption) >=
...
*
accel_find_interned_string(zend_string
*str)
{
zend_ulong h;
uint32_t pos;
...
zend_string *ZEND_FASTCALL
accel_new_interned_string(zend_string
*str)
{
zend_ulong h;
...
if (UNEXPECTED((char*)ZCSG(interned_strings).end - (char*)ZCSG(interned_strings).top <
STRTAB_STR_SIZE(str))) {
...
memcpy(ZSTR_VAL(s), ZSTR_VAL(str), ZSTR_LEN(s)
...
accel_new_interned_string_for_php(zend_string
*str)
{
...
static zend_always_inline zend_string
*
accel_find_interned_string_ex(zend_ulong
h,
...
accel_init_interned_string_for_php(const char *str, size_t size, bool permanent) {
    if (ZCG(counted)) {
...
static zend_string *ZEND_FASTCALL
accel_replace_string_by_shm_permanent(zend_string
*str)
{
zend_string *ret = accel_find_interned_string(str);
        ...
        while (tries--) {
            ...
            if (kill(mem_usage_check->l_pid, SIGKILL)) {
                if (errno == ESRCH) {
                    ...
                    success = 1;
                    zend_accel_error(ACCEL_LOG_WARNING, "Process %d died before SIGKILL was sent",
                                     mem_usage_check->l_pid);
                } else if (errno != 0) {
                    zend_accel_error(ACCEL_LOG_WARNING, "Failed to send SIGKILL to locker %d: %s",
                                     mem_usage_check->l_pid, strerror(errno));
                }
                break;
            }
            ...
            if (kill(mem_usage_check->l_pid, 0)) {
                if (errno == ESRCH) {
                    ...
                    zend_accel_error(ACCEL_LOG_WARNING, "Failed to check locker %d: %s", mem_usage_check->l_pid,
                                     strerror(errno));
                }
                break;
            }
...
static inline int accel_is_inactive(void) {
    ...
    if (ZCG(accel_directives).force_restart_timeout
        ...
        zend_accel_error(ACCEL_LOG_WARNING, "Forced restart at %ld (after "
        ZEND_LONG_FMT
        " seconds), locked by %d", (long) time(NULL), ZCG(
                accel_directives).force_restart_timeout, mem_usage_check.l_pid);
        ...
        if (EXPECTED(ZCG(cwd_key_len))) {
            ...
            if (UNEXPECTED(!cwd_str)) {
                ...
                zend_accel_error(ACCEL_LOG_INFO,
                                 "getcwd() failed for '%s' (%d), please try to set screwopcode.use_cwd to 0 in ini file",
                                 path, errno);
                return NULL;
            }
            ...
            if (ZCG(cwd_check)) {
                ...
                if (ZCG(accelerator_enabled)) {
                    ...
                    if (str) {
                        char buf[32];
                        char *res = zend_print_long_to_buf(buf + sizeof(buf) - 1,
                                                           STRTAB_STR_TO_POS(&ZCSG(interned_strings), str));
...
memcmp (ZSTR_VAL(filename)

+
ZSTR_LEN(filename)
- (sizeof(".phar")-1), ".phar", sizeof(".phar")-1) &&
!
...
static zend_persistent_script *store_script_in_file_cache(zend_persistent_script *new_persistent_script) {
    ...
    if ((char *) new_persistent_script->mem + new_persistent_script->size != (char *) ZCG(mem)) {
        zend_accel_error(
                ((char *) new_persistent_script->mem + new_persistent_script->size < (char *) ZCG(mem))
                ? ACCEL_LOG_ERROR : ACCEL_LOG_WARNING,
                "Internal error: wrong size calculation: %s start="
        ZEND_ADDR_FMT
        ", end="
        ZEND_ADDR_FMT
        ", real="
...
static zend_persistent_script *
cache_script_in_file_cache(zend_persistent_script *new_persistent_script, int *from_shared_memory) {
...
cache_script_in_shared_memory(zend_persistent_script *new_persistent_script, const char *key, unsigned int key_length,
    ...
    bucket = zend_accel_hash_update(&ZCSG(hash), ZSTR_VAL(new_persistent_script->script.filename),
                                    ZSTR_LEN(new_persistent_script->script.filename), 0, new_persistent_script);
    if (bucket) {
...
static void
persistent_error_cb(int type, const char *error_filename, const uint32_t error_lineno, zend_string *message) {
...
static zend_persistent_script *
screwopcode_compile_file(zend_file_handle *file_handle, int type, const char *key, zend_op_array **op_array_p) {
    ...
    if (file_handle->opened_path &&
        zend_accel_blacklist_is_blacklisted(&accel_blacklist, ZSTR_VAL(file_handle->opened_path),
                                            ZSTR_LEN(file_handle->opened_path))) {
        SHM_UNPROTECT();
	...
	*/
    ...
    zend_accel_move_user_functions(CG(function_table), CG(function_table)->nNumUsed - orig_functions_count,
                                   &new_persistent_script->script);
    zend_accel_move_user_classes(CG(class_table), CG(class_table)->nNumUsed - orig_class_count,
                                 &new_persistent_script->script);
    ...
    if (file_handle->opened_path) {
        ...
        new_persistent_script->script.filename = zend_string_init(file_handle->filename, strlen(file_handle->filename),
                                                                  0);
    }
...
zend_op_array *file_cache_compile_file(zend_file_handle *file_handle, int type) {
            ...
            if (!EG(current_execute_data) || !EG(current_execute_data)->opline ||
                ...
                if (zend_hash_add_empty_element(&EG(included_files), persistent_script->script.filename) != NULL) {
                    ...
                    if (persistent_script->is_phar) {
                        ...
                        memcpy(fname + sizeof("phar://") - 1, ZSTR_VAL(persistent_script->script.filename),
                               ZSTR_LEN(persistent_script->script.filename) + 1);
    ...
    persistent_script = screwopcode_compile_file(file_handle, type, NULL, &op_array);
...
zend_op_array *persistent_compile_file(zend_file_handle *file_handle, int type) {
    ...
    if (ZCG(cache_persistent_script) &&
        ...
        if (!ZCG(accel_directives).revalidate_path) {
            ...
            persistent_script = zend_accel_hash_str_find(&ZCSG(hash), key, key_length);
                ...
                is_stream_path(file_handle->filename) && !is_cacheable_stream_path(file_handle->filename))) {
            ZCG(cache_opline) = NULL;
            ZCG(cache_persistent_script) = NULL;
            return accelerator_orig_compile_file(file_handle, type);
        }
    ...
    if (persistent_script && ZCG(accel_directives).consistency_checks
        ...
        if (checksum != persistent_script->dynamic_members.checksum) {
            ...
            zend_accel_error(ACCEL_LOG_INFO, "Checksum failed for '%s':  expected=0x%08x, found=0x%08x",
                             ZSTR_VAL(persistent_script->script.filename), persistent_script->dynamic_members.checksum,
                             checksum);
            zend_shared_alloc_lock();
            if (!persistent_script->corrupted) {
                persistent_script->corrupted = 1;
    ...
    if (!persistent_script) {
        ...
        SHM_PROTECT();
        HANDLE_UNBLOCK_INTERRUPTIONS();
        persistent_script = screwopcode_compile_file(file_handle, type, key, &op_array);
        HANDLE_BLOCK_INTERRUPTIONS();
        SHM_UNPROTECT();
        ...
        if (persistent_script) {
            persistent_script = cache_script_in_shared_memory(persistent_script, key, key ? key_length : 0,
                                                              &from_shared_memory);
        }
...
zend_result accel_activate(INIT_FUNC_ARGS) {
        ...
        if (stat("/", &buf) != 0) {
            ...
            if (sizeof(buf.st_ino) > sizeof(ZCG(root_hash))) {
                if (ZCG(root_hash) != buf.st_ino) {
                    zend_string * key = zend_string_init("screwopcode.enable", sizeof("screwopcode.enable") - 1, 0);
    ...
    if (ZCSG(restart_pending)) {
        zend_shared_alloc_lock();
        if (ZCSG(restart_pending) !=
            0) { /* check again, to ensure that the cache wasn't already cleaned by another process */
...
#if defined(COMPILE_DL_SCREWOPCODE) && defined(ZTS)
...
static void accel_move_code_to_huge_pages(void)
{
	zend_error(E_WARNING, ACCELERATOR_PRODUCT_NAME ": screwopcode.huge_code_pages has no affect as huge page is not supported");
	return;
}
...
# endif
    ...
    if (accel_find_sapi() == FAILURE) {
        ...
        if (!ZCG(accel_directives).enable_cli &&
            ...
            zps_startup_failure("Opcode Caching is only supported in Apache, FPM, FastCGI and LiteSpeed SAPIs", NULL,
...
# endif
        ...
        switch (zend_shared_alloc_startup(shm_size, 0)) {
            ...
            case ALLOC_FAILURE:
                accel_startup_ok = 0;
                zend_accel_error(ACCEL_LOG_FATAL,
                                 "Failure to initialize shared memory structures - probably not enough shared memory.");
                return SUCCESS;
            ...
            case FAILED_REATTACHED:
                accel_startup_ok = 0;
                zend_accel_error(ACCEL_LOG_FATAL,
                                 "Failure to initialize shared memory structures - can not reattach to exiting shared memory.");
                return SUCCESS;
                break;
#if ENABLE_FILE_CACHE_FALLBACK
                                                                                                                                        case ALLOC_FALLBACK:
        ...
        accel_startup_ok = 0;
        zend_accel_error(ACCEL_LOG_FATAL,
                         "screwopcode.file_cache_only is set without a proper setting of screwopcode.file_cache");
        return SUCCESS;
    } else {
...
static void preload_restart(void) {
    zend_accel_hash_update(&ZCSG(hash), ZSTR_VAL(ZCSG(preload_script)->script.filename),
                           ZSTR_LEN(ZCSG(preload_script)->script.filename), 0, ZCSG(preload_script));
    if (ZCSG(saved_scripts)) {
        zend_persistent_script **p = ZCSG(saved_scripts);
        while (*p) {
            zend_accel_hash_update(&ZCSG(hash), ZSTR_VAL((*p)->script.filename), ZSTR_LEN((*p)->script.filename), 0,
                                   *p);
            p++;
        }
    }
...
static zend_bool preload_try_resolve_constants(zend_class_entry *ce) {
    ...
    do {
        ...
        if (ce->default_static_members_count) {
            uint32_t count = ce->parent ? ce->default_static_members_count - ce->parent->default_static_members_count
            ...
            if (opline->opcode == ZEND_INCLUDE_OR_EVAL &&
                ...
                if (resolved_path) {
                    ...
                    if (incl && incl->empty) {
                        ...
                        if (!IS_ABSOLUTE_PATH(Z_STRVAL_P(RT_CONSTANT(opline, opline->op1)),
                                              Z_STRLEN_P(RT_CONSTANT(opline, opline->op1)))) {
...
static int preload_optimize(zend_persistent_script *script) {
    ...
    if (!zend_optimize_script(&script->script, ZCG(accel_directives).optimization_level,
                              ZCG(accel_directives).opt_debug_level)) {
        return FAILURE;
    }
...
static zend_persistent_script *preload_script_in_shared_memory(zend_persistent_script *new_persistent_script) {
    ...
    if (zend_accel_hash_is_full(&ZCSG(hash))) {
        zend_accel_error(ACCEL_LOG_FATAL,
                         "Not enough entries in hash table for preloading. Consider increasing the value for the screwopcode.max_accelerated_files directive in php.ini.");
        return NULL;
    }
    ...
    if (!ZCG(mem)) {
        zend_accel_error(ACCEL_LOG_FATAL,
                         "Not enough shared memory for preloading. Consider increasing the value for the screwopcode.memory_consumption directive in php.ini.");
        return NULL;
    }
...
static void preload_load(void) {
    ...
    if (CG(map_ptr_last) != ZCSG(map_ptr_last)) {
        ...
        ZEND_MAP_PTR_SET_REAL_BASE(CG(map_ptr_base), perealloc(ZEND_MAP_PTR_REAL_BASE(CG(map_ptr_base)),
                                                               CG(map_ptr_size) * sizeof(void *), 1));
    ...
    if (ret == SUCCESS) {
        ...
        if (EG(zend_constants)) {
            ...
            ZEND_HASH_FOREACH_PTR(preload_scripts, script)
            {
                ...
                if ((offset = zend_get_constant_str("__COMPILER_HALT_OFFSET__",
                                                    sizeof("__COMPILER_HALT_OFFSET__") - 1)) != NULL) {
        ...
        ZEND_PASS_TWO_UPDATE_CONSTANT(&script->script.main_op_array, script->script.main_op_array.opcodes,
                                      script->script.main_op_array.opcodes[0].op1);
...
static int accel_finish_startup(void) {
    ...
    if (ZCG(accel_directives).preload && *ZCG(accel_directives).preload) {
        ...
        int (*orig_header_handler)(sapi_header_struct *sapi_header, sapi_header_op_enum op,
                                   sapi_headers_struct *sapi_headers) = sapi_module.header_handler;
        ...
        size_t(*orig_ub_write)(
        const char *str, size_t
        str_length) = sapi_module.ub_write;
        ...
        if (geteuid() == 0) {
            ...
            if (!ZCG(accel_directives).preload_user
                ...
                zend_shared_alloc_unlock();
                zend_accel_error(ACCEL_LOG_FATAL, "\"screwopcode.preload_user\" has not been defined");
                return FAILURE;
            }
            ...
            if (pw == NULL) {
                zend_shared_alloc_unlock();
                zend_accel_error(ACCEL_LOG_FATAL, "Preloading failed to getpwnam(\"%s\")",
                                 ZCG(accel_directives).preload_user);
                return FAILURE;
            }
            ...
            if (pid == -1) {
                ...
                if (initgroups(pw->pw_name, pw->pw_gid) < 0) {
                    zend_accel_error(ACCEL_LOG_WARNING, "Preloading failed to initgroups(\"%s\", %d)", pw->pw_name,
                                     pw->pw_uid);
                    exit(1);
                }
            ...
            if (ZCG(accel_directives).preload_user
                && *ZCG(accel_directives).preload_user) {
                zend_accel_error(ACCEL_LOG_WARNING, "\"screwopcode.preload_user\" is ignored");
            }
        }
