...
static void accel_interned_strings_restore_state(void)
{
	...
	while (idx > 0)
	{
		...
		if ((char *)p->key < ZCSG(interned_strings_top))
			break;
...
static inline int accel_is_inactive(void)
{
	...
	if (ZCG(accel_directives).force_restart_timeout && ZCSG(force_restart_time) && time(NULL) >= ZCSG(force_restart_time))
	{
	...
	else
	{
		...
		else
		{
			...
			if (UNEXPECTED(!cwd_str))
			{
				...
				zend_accel_error(ACCEL_LOG_INFO, "getcwd() failed for '%s' (%d), please try to set screwopcode.use_cwd to 0 in ini file", path, errno);
				return NULL;
			}
...
static zend_persistent_script *screwopcode_compile_file(zend_file_handle *file_handle, int type, char *key, zend_op_array **op_array_p)
{
...
static inline int accel_find_sapi(void)
{
	...
	if (sapi_module.name)
	{
		...
		if (ZCG(accel_directives).enable_cli && (strcmp(sapi_module.name, "cli") == 0 || strcmp(sapi_module.name, "phpdbg") == 0))
		{
			return SUCCESS;
		}
	}
...
/********************************************/
	...
	{
		accel_startup_ok = 0;
		zend_accel_error(ACCEL_LOG_FATAL, "screwopcode.file_cache_only is set without a proper setting of screwopcode.file_cache");
		return SUCCESS;
	}
	else
	{
