PHP_ARG_ENABLE([screwopcode],
  [whether to enable Screw Opcode support],
  [AS_HELP_STRING([--disable-screwopcode],
    [Disable Screw Opcode support])],
  [yes])
...
PHP_ARG_ENABLE([screwopcode-jit],
  [whether to enable JIT],
  [AS_HELP_STRING([--disable-screwopcode-jit],
    [Disable JIT])],
  [yes],
  [no])

if test "$PHP_SCREWOPCODE" != "no"; then
  ...
  if test "$PHP_SCREWOPCODE_JIT" = "yes"; then
    case $host_cpu in
      x86*)
        ;;
      *)
        AC_MSG_WARN([JIT not supported by host architecture])
        PHP_SCREWOPCODE_JIT=no
        ;;
    esac
  fi
  ...
  if test "$PHP_SCREWOPCODE_JIT" = "yes"; then
    ...
    if test -z "$OPAGENT_DIR"; then
      ...
      [
        ...
        PHP_ADD_LIBRARY_WITH_PATH(opagent, $OPAGENT_DIR/$PHP_LIBDIR/oprofile, SCREWOPCODE_SHARED_LIBADD)
        PHP_SUBST(SCREWOPCODE_SHARED_LIBADD)
      ],[
        AC_MSG_RESULT(not found)
      ],[
...
int main() {
  ...
  sprintf(tmpname,"/screwopcode.test.shm.%dXXXXXX", getpid());
  ...
  return 0;
    ...
    PHP_CHECK_LIBRARY(rt, shm_unlink, [PHP_ADD_LIBRARY(rt,1,SCREWOPCODE_SHARED_LIBADD)])
  ],[
    AC_MSG_RESULT([no])
  ],[
    AC_MSG_RESULT([no])
  ...
  PHP_ADD_EXTENSION_DEP(screwopcode, pcre)

  if test "$have_shm_ipc" != "yes" && test "$have_shm_mmap_posix" != "yes" && test "$have_shm_mmap_anon" != "yes"; then
    AC_MSG_ERROR([No supported shared memory caching support was found when configuring screwopcode. Check config.log for any errors or missing dependencies.])
  fi
