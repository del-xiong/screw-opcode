...
static zend_never_inline zend_op_array
*

ZEND_FASTCALL zend_jit_init_func_run_time_cache_helper(zend_op_array *op_array) /* {{{ */
{
    void **run_time_cache;
...
static zend_function *ZEND_FASTCALL
zend_jit_find_func_helper(zend_string
*name)
{
...
if (
EXPECTED(fbc
->type == ZEND_USER_FUNCTION) && UNEXPECTED(!RUN_TIME_CACHE(&fbc->op_array))) {
...
static zend_function *ZEND_FASTCALL
zend_jit_find_ns_func_helper(zval
*func_name)
{
...
static ZEND_COLD void ZEND_FASTCALL
zend_jit_invalid_method_call(zval
*object)
{
zend_execute_data *execute_data = EG(current_execute_data);
...
if (
Z_TYPE_P(object)
== IS_UNDEF && opline->op1_type == IS_CV) {
...
static ZEND_COLD void ZEND_FASTCALL
zend_jit_invalid_method_call_tmp(zval
*object)
{
zend_execute_data *execute_data = EG(current_execute_data);
const zend_op *opline = EX(opline);
...
zval_ptr_dtor_nogc (EX_VAR(opline

->op1.var));
}

static zend_never_inline ZEND_COLD
void ZEND_FASTCALL
...
static void ZEND_FASTCALL
zend_jit_unref_helper(zval
*zv)
{
zend_reference *ref;
...
static zend_function *ZEND_FASTCALL
zend_jit_find_method_helper(zend_object
*obj,
zval *function_name, zend_object
**obj_ptr)
{
zend_execute_data *execute_data = EG(current_execute_data);
const zend_op *opline = EX(opline);
...
CACHE_POLYMORPHIC_PTR(opline
->result.num, called_scope, fbc);
}
...
static zend_function *ZEND_FASTCALL
zend_jit_find_method_tmp_helper(zend_object
*obj,
zval *function_name, zend_object
**obj_ptr)
{
...
static zend_execute_data *ZEND_FASTCALL
zend_jit_push_static_metod_call_frame(zend_object
*obj,
zend_function *fbc, uint32_t
num_args)
{
zend_class_entry *scope = obj->ce;

return
zend_vm_stack_push_call_frame(ZEND_CALL_NESTED_FUNCTION, fbc, num_args, scope
);
}

static zend_execute_data *ZEND_FASTCALL
zend_jit_push_static_metod_call_frame_tmp(zend_object
*obj,
zend_function *fbc, uint32_t
num_args)
{
...
static zend_execute_data *ZEND_FASTCALL
zend_jit_extend_stack_helper(uint32_t
used_stack,
zend_function *fbc
)
{
...
static zend_execute_data *ZEND_FASTCALL
zend_jit_int_extend_stack_helper(uint32_t
used_stack)
{
zend_execute_data *call = (zend_execute_data *) zend_vm_stack_extend(used_stack);
ZEND_CALL_INFO(call) = ZEND_CALL_NESTED_FUNCTION | ZEND_CALL_ALLOCATED;
...
static zval *ZEND_FASTCALL
zend_jit_symtable_find(HashTable
*ht,
zend_string *str
)
{
...
if (
_zend_handle_numeric_str_ex(str
->val, str->len, &idx)) {
return
zend_hash_index_find(ht, idx
);
}
} while (0);

return
zend_hash_find(ht, str
);
}

static zval *ZEND_FASTCALL
zend_jit_hash_index_lookup_rw(HashTable
*ht,
zend_long idx
)
{
...
UNEXPECTED (zend_undefined_offset_write(ht, idx)
...
static zval *ZEND_FASTCALL
zend_jit_hash_index_lookup_w(HashTable
*ht,
zend_long idx
)
{
...
static zval *ZEND_FASTCALL
zend_jit_hash_lookup_rw(HashTable
*ht,
zend_string *str
)
{
...
UNEXPECTED (zend_undefined_index_write(ht, str)
...
static zval *ZEND_FASTCALL
zend_jit_hash_lookup_w(HashTable
*ht,
zend_string *str
)
{
...
static zval *ZEND_FASTCALL
zend_jit_symtable_lookup_rw(HashTable
*ht,
zend_string *str
)
{
...
static zval *ZEND_FASTCALL
zend_jit_symtable_lookup_w(HashTable
*ht,
zend_string *str
)
{
...
zend_jit_undefined_op_helper(uint32_t
var)
{
...
static int ZEND_FASTCALL
zend_jit_undefined_op_helper_write(HashTable
*ht,
uint32_t var
)
{
...
static void ZEND_FASTCALL
zend_jit_fetch_dim_r_helper(zend_array
*ht,
zval *dim, zval
*result)
{
...
zend_jit_undefined_op_helper (EG(current_execute_data)
...
str_index:
if (
ZEND_HANDLE_NUMERIC(offset_key, hval
)) {
goto
num_index;
}
...
num_undef:
zend_error(E_WARNING,
"Undefined array key " ZEND_LONG_FMT, hval);
ZVAL_NULL(result);
}

static void ZEND_FASTCALL
zend_jit_fetch_dim_is_helper(zend_array
*ht,
zval *dim, zval
*result)
{
...
static int ZEND_FASTCALL
zend_jit_fetch_dim_isset_helper(zend_array
*ht,
zval *dim
)
{
...
static zval *ZEND_FASTCALL
zend_jit_fetch_dim_rw_helper(zend_array
*ht,
zval *dim
)
{
...
case IS_UNDEF:
if (!
zend_jit_undefined_op_helper_write(ht, EG(current_execute_data)
->opline->op2.var)) {
return
NULL;
}
...
UNEXPECTED (zend_undefined_index_write(ht, offset_key)
...
UNEXPECTED (zend_undefined_offset_write(ht, hval)
...
static zval *ZEND_FASTCALL
zend_jit_fetch_dim_w_helper(zend_array
*ht,
zval *dim
)
{
...
static zend_never_inline zend_long
zend_check_string_offset(zval
*dim/*, int type*/)
{
zend_long offset;
...
is_numeric_string_ex(Z_STRVAL_P(dim), Z_STRLEN_P(dim),

&offset, NULL,
/* allow errors */ true, NULL, &trailing_data)) {
if (
UNEXPECTED(trailing_data)
&&
...
return
_zval_get_long_func(dim);
}

static zend_always_inline zend_string
*
zend_jit_fetch_dim_str_offset(zend_string
*str,
zend_long offset
)
{
if (
UNEXPECTED((zend_ulong)
offset >= (zend_ulong)
ZSTR_LEN(str)
)) {
if (
...
zend_long real_offset = (zend_long)
ZSTR_LEN(str)
+
offset;

if (
EXPECTED(real_offset
>= 0)) {
return
ZSTR_CHAR((zend_uchar)
ZSTR_VAL(str)[real_offset]
);
}
}
zend_error(E_WARNING,
"Uninitialized string offset " ZEND_LONG_FMT, offset);
return
...
zend_jit_fetch_dim_str_offset_r_helper(zend_string
*str,
zend_long offset
)
{
return
zend_jit_fetch_dim_str_offset(str, offset
);
}

static zend_string *ZEND_FASTCALL
zend_jit_fetch_dim_str_r_helper(zend_string
*str,
zval *dim
)
{
...
static void ZEND_FASTCALL
zend_jit_fetch_dim_str_is_helper(zend_string
*str,
zval *dim, zval
*result)
{
...
is_numeric_string(Z_STRVAL_P(dim), Z_STRLEN_P(dim), NULL, NULL, false
...
if ((zend_ulong)offset >= (zend_ulong)
ZSTR_LEN(str)
) {
if (offset < 0) {
/* Handle negative offset */
...
static void ZEND_FASTCALL
zend_jit_fetch_dim_obj_r_helper(zval
*container,
zval *dim, zval
*result)
{
...
static void ZEND_FASTCALL
zend_jit_fetch_dim_obj_is_helper(zval
*container,
zval *dim, zval
*result)
{
...
void zend_wrong_string_offset(void) {
    const char *msg = NULL;
...
static zend_always_inline void ZEND_FASTCALL
zend_jit_fetch_dim_obj_helper(zval
*object_ptr,
zval *dim, zval
*result,
int type
...
UNEXPECTED (Z_REFCOUNT_P(retval)
...
&& "read_dimension() returned NULL without exception");
ZVAL_UNDEF(result);
}
} else if (
...
static void ZEND_FASTCALL
zend_jit_fetch_dim_obj_w_helper(zval
*object_ptr,
zval *dim, zval
*result)
{
...
static void ZEND_FASTCALL
zend_jit_fetch_dim_obj_rw_helper(zval
*object_ptr,
zval *dim, zval
*result)
{
...
static void ZEND_FASTCALL
zend_jit_assign_dim_helper(zval
*object_ptr,
zval *dim, zval
*value,
zval *result
...
== IS_OBJECT)) {
ZVAL_DEREF(value);
Z_OBJ_HT_P(object_ptr)
->

write_dimension(Z_OBJ_P(object_ptr), dim, value
...
static void ZEND_FASTCALL
zend_jit_assign_dim_op_helper(zval
*container,
zval *dim, zval
*value,
...
write_dimension(Z_OBJ_P(object), property,
...
static void ZEND_FASTCALL
zend_jit_fast_assign_concat_helper(zval
*op1,
zval *op2
)
{
...
memcpy (ZSTR_VAL(result_str)

+ op1_len,
Z_STRVAL_P(op2), op2_len
);
ZSTR_VAL(result_str)[result_len] = '\0';
}

static void ZEND_FASTCALL
zend_jit_fast_concat_helper(zval
*result,
zval *op1, zval
*op2)
{
...
static int ZEND_FASTCALL
zend_jit_isset_dim_helper(zval
*container,
zval *offset
)
{
...
== IS_OBJECT)) {
return
Z_OBJ_HT_P(container)
->

has_dimension(Z_OBJ_P(container), offset,
...
== IS_STRING)) { /* string offsets */
zend_long lval;

if (

EXPECTED (Z_TYPE_P(offset)
...
if (
UNEXPECTED(lval<
0)) { /* Handle negative offset */
lval += (zend_long)
Z_STRLEN_P(container);
}
if (
EXPECTED(lval
>= 0) && (size_t)lval <
...
if (
Z_TYPE_P(offset)
< IS_STRING /* simple scalar types */
|| (
Z_TYPE_P(offset)
== IS_STRING /* or numeric string */
&& IS_LONG ==

is_numeric_string(Z_STRVAL_P(offset), Z_STRLEN_P(offset), NULL, NULL, false
...
static void ZEND_FASTCALL
zend_jit_free_call_frame(zend_execute_data
*call)
{
zend_vm_stack_free_call_frame(call);
}

static zend_reference *ZEND_FASTCALL
zend_jit_fetch_global_helper(zend_string
*varname,
void **cache_slot
)
{
...
EXPECTED (zend_string_equal_content(p
...
zend_jit_verify_type_common(zval
*arg,
...
if (
ZEND_TYPE_HAS_CLASS(arg_info
->type) &&
Z_TYPE_P(arg)
== IS_OBJECT) {
zend_class_entry *ce;
if (
ZEND_TYPE_HAS_LIST(arg_info
->type)) {
zend_type *list_type;

ZEND_TYPE_LIST_FOREACH (ZEND_TYPE_LIST(arg_info
...
zend_value_instanceof_static(arg)
) {
return 1;
}
if (
zend_verify_scalar_type_hint(type_mask, arg, ZEND_ARG_USES_STRICT_TYPES(), /* is_internal */ 0)) {
return 1;
}
return 0;
}

static zend_bool ZEND_FASTCALL
zend_jit_verify_arg_slow(zval
*arg,
zend_arg_info *arg_info
)
{
...
if (UNEXPECTED(!ret)) {
zend_verify_arg_error(EX(func), arg_info, opline
...
static void ZEND_FASTCALL
zend_jit_verify_return_slow(zval
*arg,
const zend_op_array *op_array, zend_arg_info
*arg_info,
void **cache_slot
)
{
if (UNEXPECTED(!
zend_jit_verify_type_common(arg, arg_info, cache_slot
))) {
zend_verify_return_error((zend_function
*)op_array, arg);
}
}

static void ZEND_FASTCALL
zend_jit_fetch_obj_r_slow(zend_object
*zobj)
{
zval *retval;
zend_execute_data *execute_data = EG(current_execute_data);
...
static void ZEND_FASTCALL
zend_jit_fetch_obj_r_dynamic(zend_object
*zobj,
intptr_t prop_offset
)
{
if (zobj->properties) {
zval *retval;
...
if (!
IS_UNKNOWN_DYNAMIC_PROPERTY_OFFSET(prop_offset)
) {
intptr_t idx = ZEND_DECODE_DYN_PROP_OFFSET(prop_offset);

if (
EXPECTED(idx<zobj
->properties->nNumUsed * sizeof(Bucket))) {
...
EXPECTED (memcmp(ZSTR_VAL(p

->key),
ZSTR_VAL(name), ZSTR_LEN(name)
) == 0)))) {
ZVAL_COPY_DEREF(result,
&p->val);
return;
}
}
CACHE_PTR_EX(cache_slot
+ 1, (void*)ZEND_DYNAMIC_PROPERTY_OFFSET);
}
...
CACHE_PTR_EX(cache_slot
+ 1, (void*)
ZEND_ENCODE_DYN_PROP_OFFSET(idx)
);
ZVAL_COPY_DEREF(result, retval
);
return;
...
static void ZEND_FASTCALL
zend_jit_fetch_obj_is_slow(zend_object
*zobj)
{
zval *retval;
zend_execute_data *execute_data = EG(current_execute_data);
...
static void ZEND_FASTCALL
zend_jit_fetch_obj_is_dynamic(zend_object
*zobj,
intptr_t prop_offset
)
{
...
Z_TYPE_P (Z_REFVAL_P(val))

<= IS_FALSE);
}

static zend_always_inline zend_bool
check_type_array_assignable(zend_type
type) {
if (!
ZEND_TYPE_IS_SET(type)
) {
return 1;
}
return (
ZEND_TYPE_FULL_MASK(type)
& (MAY_BE_ITERABLE|MAY_BE_ARRAY)) != 0;
}
...
void zend_throw_auto_init_in_prop_error(zend_property_info *prop, const char *type) {
...
void zend_throw_access_uninit_prop_by_ref_error(
...
static zend_never_inline zend_bool
zend_handle_fetch_obj_flags(
        zval
*result,
zval *ptr, zend_object
*obj,
zend_property_info *prop_info, uint32_t
flags)
{
switch (flags) {
case ZEND_FETCH_DIM_WRITE:
if (
promotes_to_array(ptr)
) {
if (!prop_info) {
...
if (!
check_type_array_assignable(prop_info
->type)) {
...
== IS_UNDEF) {
if (!
ZEND_TYPE_ALLOW_NULL(prop_info
->type)) {
...
static void ZEND_FASTCALL
zend_jit_fetch_obj_w_slow(zend_object
*zobj)
{
zval *retval;
zend_execute_data *execute_data = EG(current_execute_data);
...
if (UNEXPECTED(!
zend_handle_fetch_obj_flags(result, retval, zobj, prop_info, flags
))) {
return;
}
}
...
static void ZEND_FASTCALL
zend_jit_check_array_promotion(zval
*val,
zend_property_info *prop
)
{
...
<= IS_FALSE))
&&
ZEND_TYPE_IS_SET(prop
->type)
&& (
ZEND_TYPE_FULL_MASK(prop
->type) & (MAY_BE_ITERABLE|MAY_BE_ARRAY)) == 0) {
zend_string *type_str = zend_type_to_string(prop->type);
zend_type_error(
"Cannot auto-initialize an array inside property %s::$%s of type %s",
ZSTR_VAL(prop
->ce->name),
zend_get_unmangled_property_name(prop
->name),
ZSTR_VAL(type_str)
);
zend_string_release(type_str);
...
static void ZEND_FASTCALL
zend_jit_create_typed_ref(zval
*val,
zend_property_info *prop, zval
*result)
{
if (!
Z_ISREF_P(val)
...
static void ZEND_FASTCALL
zend_jit_extract_helper(zend_refcounted
*garbage)
{
zend_execute_data *execute_data = EG(current_execute_data);
const zend_op *opline = execute_data->opline;
...
static void ZEND_FASTCALL
zend_jit_vm_stack_free_args_helper(zend_execute_data
*call)
{
...
zend_jit_assign_to_typed_ref_helper(zend_reference *ref, zval *value, zend_uchar value_type) {
    zval variable;
...
static void ZEND_FASTCALL
zend_jit_assign_const_to_typed_ref(zend_reference
*ref,
zval *value
)
{
...
static void ZEND_FASTCALL
zend_jit_assign_tmp_to_typed_ref(zend_reference
*ref,
zval *value
)
{
...
static void ZEND_FASTCALL
zend_jit_assign_var_to_typed_ref(zend_reference
*ref,
zval *value
)
{
...
static void ZEND_FASTCALL
zend_jit_assign_cv_to_typed_ref(zend_reference
*ref,
zval *value
)
{
...
static void ZEND_FASTCALL
zend_jit_pre_inc_typed_ref(zend_reference
*ref,
zval *ret
)
{
...
UNEXPECTED (Z_TYPE_P(var_ptr)

== IS_DOUBLE) &&
Z_TYPE(tmp)
== IS_LONG) {
zend_jit_throw_incdec_ref_error(ref,
1);
ZVAL_COPY_VALUE(var_ptr,
&tmp);
} else if (UNEXPECTED(!
zend_verify_ref_assignable_zval(ref, var_ptr, ZEND_CALL_USES_STRICT_TYPES(EG(current_execute_data))
))) {
...
static void ZEND_FASTCALL
zend_jit_pre_dec_typed_ref(zend_reference
*ref,
zval *ret
)
{
...
static void ZEND_FASTCALL
zend_jit_post_inc_typed_ref(zend_reference
*ref,
zval *ret
)
{
...
static void ZEND_FASTCALL
zend_jit_post_dec_typed_ref(zend_reference
*ref,
zval *ret
)
{
...
static void ZEND_FASTCALL
zend_jit_assign_op_to_typed_ref(zend_reference
*ref,
zval *val, binary_op_type
binary_op)
{
zval z_copy;
...
EXPECTED (zend_verify_ref_assignable_zval(ref,
...
static void ZEND_FASTCALL
zend_jit_only_vars_by_reference(zval
*arg)
{
...
static void ZEND_FASTCALL
zend_jit_invalid_array_access(zval
*container)
{
zend_error(E_WARNING,
...
static void ZEND_FASTCALL
zend_jit_invalid_property_read(zval
*container,
const char *property_name
)
{
zend_error(E_WARNING,
"Attempt to read property \"%s\" on %s", property_name,
zend_zval_type_name(container)
);
}

static void ZEND_FASTCALL
zend_jit_invalid_property_write(zval
*container,
const char *property_name
)
{
...
static void ZEND_FASTCALL
zend_jit_invalid_property_incdec(zval
*container,
const char *property_name
)
{
...
static void ZEND_FASTCALL
zend_jit_invalid_property_assign(zval
*container,
const char *property_name
)
{
...
static void ZEND_FASTCALL
zend_jit_invalid_property_assign_op(zval
*container,
const char *property_name
)
{
...
zend_jit_undefined_op_helper (EX(opline)
...
static zval *ZEND_FASTCALL
zend_jit_prepare_assign_dim_ref(zval
*ref) {
...
ZEND_REF_HAS_TYPE_SOURCES (Z_REF_P(ref))
...
static zend_never_inline void ZEND_FASTCALL
_zend_hash_iterators_remove(HashTable
*ht)
{
...
static void ZEND_FASTCALL
zend_jit_array_free(HashTable
*ht)
{
GC_REMOVE_FROM_BUFFER(ht);
if (

UNEXPECTED (HT_HAS_ITERATORS(ht))
...
static HashTable *ZEND_FASTCALL
zend_jit_zval_array_dup(zval
*arr)
{
HashTable *ht;
...
zend_jit_add_arrays_helper(zend_array
*op1,
zend_array *op2
)
{
...
zend_hash_merge(res, op2, zval_add_ref,
0);
return
res;
}
...
static void ZEND_FASTCALL
zend_jit_assign_obj_helper(zend_object
*zobj,
zend_string *name, zval
*value,
void **cache_slot, zval
*result)
{
ZVAL_DEREF(value);
...
static void ZEND_FASTCALL
zend_jit_assign_to_typed_prop(zval
*property_val,
zend_property_info *info, zval
*value,
zval *result
)
{
...
if (UNEXPECTED(!
zend_verify_property_type(info,
&tmp,
...
static zend_never_inline void
_zend_jit_assign_op_overloaded_property(zend_object *object, zend_string *name, void **cache_slot, zval *value,
                                        binary_op_type binary_op) {
    zval * z;
    zval rv, res;
...
static void ZEND_FASTCALL
zend_jit_assign_op_to_typed_prop(zval
*zptr,
zend_property_info *prop_info, zval
*value,
binary_op_type binary_op
)
{
...
EXPECTED (zend_verify_property_type(prop_info,
...
static void ZEND_FASTCALL
zend_jit_assign_obj_op_helper(zend_object
*zobj,
zend_string *name, zval
*value,
void **cache_slot, binary_op_type
binary_op)
{
zval *zptr;
...
if (
EXPECTED((zptr = zobj->handlers->get_property_ptr_ptr(zobj, name, BP_VAR_RW, cache_slot)
) != NULL)) {
if (

UNEXPECTED (Z_ISERROR_P(zptr))
...
static ZEND_COLD zend_long
_zend_jit_throw_inc_prop_error(zend_property_info
*prop)
{
zend_string *type_str = zend_type_to_string(prop->type);
...
static ZEND_COLD zend_long
_zend_jit_throw_dec_prop_error(zend_property_info
*prop)
{
zend_string *type_str = zend_type_to_string(prop->type);
...
zend_verify_property_type(prop_info, var_ptr, EX_USES_STRICT_TYPES()
))) {
zval_ptr_dtor(var_ptr);
ZVAL_COPY_VALUE(var_ptr,
&tmp);
...
static void ZEND_FASTCALL
zend_jit_pre_inc_typed_prop(zval
*var_ptr,
zend_property_info *prop_info, zval
*result)
{
...
static void ZEND_FASTCALL
zend_jit_pre_dec_typed_prop(zval
*var_ptr,
zend_property_info *prop_info, zval
*result)
{
...
static void ZEND_FASTCALL
zend_jit_post_inc_typed_prop(zval
*var_ptr,
zend_property_info *prop_info, zval
*result)
{
...
static void ZEND_FASTCALL
zend_jit_post_dec_typed_prop(zval
*var_ptr,
zend_property_info *prop_info, zval
*result)
{
...
static void ZEND_FASTCALL
zend_jit_pre_inc_obj_helper(zend_object
*zobj,
zend_string *name,
void **cache_slot, zval
*result)
...
if (
EXPECTED((prop = zobj->handlers->get_property_ptr_ptr(zobj, name, BP_VAR_RW, cache_slot)
) != NULL)) {
if (

UNEXPECTED (Z_ISERROR_P(prop))
...
zend_property_info *prop_info = (zend_property_info * )
CACHED_PTR_EX(cache_slot
+ 2);

if (

EXPECTED (Z_TYPE_P(prop)

== IS_LONG)) {
fast_long_increment_function(prop);
if (

UNEXPECTED (Z_TYPE_P(prop)
...
static void ZEND_FASTCALL
zend_jit_pre_dec_obj_helper(zend_object
*zobj,
zend_string *name,
void **cache_slot, zval
*result)
...
static void ZEND_FASTCALL
zend_jit_post_inc_obj_helper(zend_object
*zobj,
zend_string *name,
void **cache_slot, zval
*result)
...
static void ZEND_FASTCALL
zend_jit_post_dec_obj_helper(zend_object
*zobj,
zend_string *name,
void **cache_slot, zval
*result)
...
static void ZEND_FASTCALL
zend_jit_free_trampoline_helper(zend_function
*func)
{
ZEND_ASSERT(func
->common.fn_flags & ZEND_ACC_CALL_VIA_TRAMPOLINE);
zend_string_release_ex(func
