...
void ZEND_FASTCALL
zend_jit_hot_func(zend_execute_data
*execute_data,
...
# define OPLINE_DC
# define OPLINE_CC
# define ZEND_SCREWOPCODE_HANDLER_RET              void
# define ZEND_SCREWOPCODE_HANDLER_ARGS             EXECUTE_DATA_D
# define ZEND_SCREWOPCODE_HANDLER_ARGS_PASSTHRU
# define ZEND_SCREWOPCODE_HANDLER_ARGS_DC
# define ZEND_SCREWOPCODE_HANDLER_ARGS_PASSTHRU_CC
# define ZEND_SCREWOPCODE_RETURN()                 return
# define ZEND_SCREWOPCODE_TAIL_CALL(handler)       do { \
        handler(ZEND_SCREWOPCODE_HANDLER_ARGS_PASSTHRU); \
        return; \
    } while(0)
# define ZEND_SCREWOPCODE_TAIL_CALL_EX(handler, arg) do { \
        handler(arg ZEND_SCREWOPCODE_HANDLER_ARGS_PASSTHRU_CC); \
        return; \
    } while(0)
#else
...
# define ZEND_SCREWOPCODE_HANDLER_RET              int
# define ZEND_SCREWOPCODE_HANDLER_ARGS             EXECUTE_DATA_D
# define ZEND_SCREWOPCODE_HANDLER_ARGS_PASSTHRU    EXECUTE_DATA_C
# define ZEND_SCREWOPCODE_HANDLER_ARGS_DC          EXECUTE_DATA_DC
# define ZEND_SCREWOPCODE_HANDLER_ARGS_PASSTHRU_CC EXECUTE_DATA_CC
# define ZEND_SCREWOPCODE_RETURN()                 return 0
# define ZEND_SCREWOPCODE_TAIL_CALL(handler)       do { \
        return handler(ZEND_SCREWOPCODE_HANDLER_ARGS_PASSTHRU); \
    } while(0)
# define ZEND_SCREWOPCODE_TAIL_CALL_EX(handler, arg) do { \
        return handler(arg ZEND_SCREWOPCODE_HANDLER_ARGS_PASSTHRU_CC); \
    } while(0)
#endif

/* VM handlers */
typedef ZEND_SCREWOPCODE_HANDLER_RET (ZEND_FASTCALL
*zend_vm_opcode_handler_t)(
ZEND_SCREWOPCODE_HANDLER_ARGS
);
...
static zend_always_inline const zend_op
*
zend_jit_trace_get_exit_opline(zend_jit_trace_rec
*trace,
const zend_op *opline, zend_bool
*exit_if_true)
{
...
return
OP_JMP_ADDR(opline, opline
->op2);
} else if (trace->opline ==
OP_JMP_ADDR(opline, opline
->op2)) {
/* taken branch */
*
...
zend_jit_may_be_polymorphic_call(const zend_op *opline) {
