 ...
 *
 * perf record php -d screwopcode.huge_code_pages=0 -d screwopcode.jit_debug=0x10 bench.php
 * perf report
 *
 * 2) Profile using jit-<pid>.dump
 *
 * perf record -k 1 php -d screwopcode.huge_code_pages=0 -d screwopcode.jit_debug=0x20 bench.php
...
static void zend_jit_perf_jitdump_register(const char *name, void *start, size_t size) {
    if (jitdump_fd >= 0) {
        ...
        rec.vma = (uint64_t)(uintptr_t)
        start;
        rec.code_address = (uint64_t)(uintptr_t)
        start;
