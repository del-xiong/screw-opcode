...
static int zend_jit_trace_startup(void) {
    ...
    zend_jit_exit_groups = (const void **) zend_shared_alloc(
            sizeof(void *) * (ZEND_JIT_TRACE_MAX_EXITS / ZEND_JIT_EXIT_POINTS_PER_GROUP));
        ...
        do {
            ...
            if (stack_size == 0
                || (t->exit_info[i].stack_size >= stack_size
                    && memcmp(t->stack_map + t->exit_info[i].stack_offset, stack,
                              stack_size * sizeof(zend_jit_trace_stack)) == 0)) {
...
zend_jit_trace_type_to_info_ex(zend_uchar
type,
uint32_t info
)
{
...
static zend_always_inline uint32_t
zend_jit_trace_type_to_info(zend_uchar
type)
{
return
zend_jit_trace_type_to_info_ex(type,
-1);
}

static zend_always_inline zend_ssa_alias_kind

zend_jit_var_may_alias(const zend_op_array *op_array, const zend_ssa *ssa, uint32_t var) {
...
zend_jit_trace_frame_size(const zend_op_array *op_array) {
    if (op_array && op_array->type == ZEND_USER_FUNCTION) {
        return ZEND_MM_ALIGNED_SIZE(offsetof(zend_jit_trace_stack_frame, stack) + ZEND_MM_ALIGNED_SIZE(
                (op_array->last_var + op_array->T) * sizeof(zend_jit_trace_stack)));
    } else {
...
zend_jit_trace_call_frame(zend_jit_trace_stack_frame *frame, const zend_op_array *op_array) {
        ...
        do {
            ...
            if (JIT_G(opt_level) >= ZEND_JIT_LEVEL_OPT_FUNCS) {
                if (zend_analyze_calls(&CG(arena), script, ZEND_CALL_TREE, (zend_op_array *) op_array,
                                       &jit_extension->func_info) != SUCCESS) {
                    break;
                }
                jit_extension->func_info.call_map = zend_build_call_map(&CG(arena), &jit_extension->func_info,
                                                                        op_array);
            ...
            if (JIT_G(debug) & ZEND_JIT_DEBUG_SSA) {
                zend_dump_op_array(op_array, ZEND_DUMP_HIDE_UNREACHABLE | ZEND_DUMP_RC_INFERENCE | ZEND_DUMP_SSA, "JIT",
                                   ssa);
            }
            return ssa;
        } while (0);
...
static int zend_jit_trace_add_phis(zend_jit_trace_rec *trace_buffer, uint32_t ssa_vars_count, zend_ssa *tssa,
...
static int zend_jit_trace_add_call_phis(zend_jit_trace_rec *trace_buffer, uint32_t ssa_vars_count, zend_ssa *tssa,
                                        zend_jit_trace_stack *stack) {
    zend_ssa_phi *prev = NULL;
...
static int zend_jit_trace_add_ret_phis(zend_jit_trace_rec *trace_buffer, uint32_t ssa_vars_count, zend_ssa *tssa,
                                       zend_jit_trace_stack *stack) {
...
static int
zend_jit_trace_copy_ssa_var_info(const zend_op_array *op_array, const zend_ssa *ssa, const zend_op **tssa_opcodes,
...
static int
zend_jit_trace_copy_ssa_var_range(const zend_op_array *op_array, const zend_ssa *ssa, const zend_op **tssa_opcodes,
                                  zend_ssa *tssa, int ssa_var) {
    int def;
    zend_ssa_op *op;
    zend_ssa_var_info *info;
    ...
    if (def >= 0) {
        ...
        if (info->has_range) {
            if (tssa->var_info[ssa_var].has_range) {
                ...
                tssa->var_info[ssa_var].range.underflow =
                        tssa->var_info[ssa_var].range.underflow && info->range.underflow;
...
static int
zend_jit_trace_restrict_ssa_var_info(const zend_op_array *op_array, const zend_ssa *ssa, const zend_op **tssa_opcodes,
                                     zend_ssa *tssa, int ssa_var) {
    int def;
    zend_ssa_op *op;
    zend_ssa_var_info *info;
...
static zend_ssa *zend_jit_trace_build_tssa(zend_jit_trace_rec *trace_buffer, uint32_t parent_trace, uint32_t exit_num,
                                           zend_script *script, const zend_op_array **op_arrays,
                                           int *num_op_arrays_ptr) {
    zend_ssa *tssa;
    ...
    *num_op_arrays_ptr = num_op_arrays;
    ...
    JIT_G(current_frame) = frame = (zend_jit_trace_stack_frame * )(
            (char *) zend_arena_alloc(&CG(arena), stack_bottom + stack_size) + stack_bottom);
    ...
    for (;; p++) {
        if (p->op == ZEND_JIT_TRACE_VM) {
            ...
            ssa_vars_count = zend_ssa_rename_op(op_array, opline, idx, build_flags, ssa_vars_count, ssa_ops,
                                                (int *) stack);
            idx++;
            ...
            if (i < op_array->num_args) {
                if (ssa->var_info
                    ...
                    if (op_array->arg_info) {
                        ...
                        ssa_var_info[i].type =
                                MAY_BE_RC1 | MAY_BE_RCN | MAY_BE_REF | MAY_BE_ANY | MAY_BE_ARRAY_KEY_ANY |
                ...
                if (ssa_vars[i].alias == NO_ALIAS) {
                    ...
                    ssa_var_info[i].type =
                            MAY_BE_UNDEF | MAY_BE_RC1 | MAY_BE_RCN | MAY_BE_REF | MAY_BE_ANY | MAY_BE_ARRAY_KEY_ANY |
                            MAY_BE_ARRAY_OF_ANY | MAY_BE_ARRAY_OF_REF;
                }
            }
            i++;
        ...
        while (i < op_array->last_var + op_array->T) {
            if (!ssa->var_info
                ...
                if (i < op_array->last_var) {
                    ...
                    ssa_var_info[i].type = MAY_BE_RC1 | MAY_BE_RCN | MAY_BE_REF | MAY_BE_ANY | MAY_BE_ARRAY_KEY_ANY |
                                           MAY_BE_ARRAY_OF_ANY | MAY_BE_ARRAY_OF_REF;
                }
            }
...
//				case ZEND_FETCH_DIM_UNSET:
                ...
                propagate_arg:
                    ...
                    if (frame->call
                        ...
                        if (ARG_SHOULD_BE_SENT_BY_REF(frame->call->func, opline->op2.num)) {
                            info |= MAY_BE_REF | MAY_BE_RC1 | MAY_BE_RCN | MAY_BE_ANY | MAY_BE_ARRAY_OF_ANY |
            ...
            if (ssa->var_info) {
                ...
                if (len == 2 && (opline + 1)->opcode == ZEND_OP_DATA) {
                    ...
                    if (ssa_ops[idx + 1].result_def >= 0) {
                        zend_jit_trace_copy_ssa_var_range(op_array, ssa, ssa_opcodes, tssa,
                                                          ssa_ops[idx + 1].result_def);
                    }
                }
            } else {
                if (ssa_ops[idx].op1_def >= 0) {
                    ssa_vars[ssa_ops[idx].op1_def].alias = zend_jit_var_may_alias(op_array, ssa,
                                                                                  EX_VAR_TO_NUM(opline->op1.var));
                }
                if (ssa_ops[idx].op2_def >= 0) {
                    ssa_vars[ssa_ops[idx].op2_def].alias = zend_jit_var_may_alias(op_array, ssa,
                                                                                  EX_VAR_TO_NUM(opline->op2.var));
                }
                if (ssa_ops[idx].result_def >= 0) {
                    ssa_vars[ssa_ops[idx].result_def].alias = zend_jit_var_may_alias(op_array, ssa,
                                                                                     EX_VAR_TO_NUM(opline->result.var));
                }
                if (len == 2 && (opline + 1)->opcode == ZEND_OP_DATA) {
                    if (ssa_ops[idx + 1].op1_def >= 0) {
                        ssa_vars[ssa_ops[idx + 1].op1_def].alias = zend_jit_var_may_alias(op_array, ssa, EX_VAR_TO_NUM(
                                (opline + 1)->op1.var));
                    }
                    if (ssa_ops[idx + 1].op2_def >= 0) {
                        ssa_vars[ssa_ops[idx + 1].op2_def].alias = zend_jit_var_may_alias(op_array, ssa, EX_VAR_TO_NUM(
                                (opline + 1)->op2.var));
                    }
                    if (ssa_ops[idx + 1].result_def >= 0) {
                        ssa_vars[ssa_ops[idx + 1].result_def].alias = zend_jit_var_may_alias(op_array, ssa,
                                                                                             EX_VAR_TO_NUM((opline +
                                                                                                            1)->result.var));
                    }
                }
            }
            if (opline->opcode == ZEND_RECV_INIT
                ...
                if (zend_update_type_info(op_array, tssa, script, (zend_op *) opline, ssa_ops + idx, ssa_opcodes,
            ...
            while (len > 1) {
                ...
                if (opline->opcode != ZEND_OP_DATA) {
                    if (ssa->var_info) {
                        ...
                        if (ssa_ops[idx].result_def >= 0) {
                            zend_jit_trace_copy_ssa_var_range(op_array, ssa, ssa_opcodes, tssa,
                                                              ssa_ops[idx].result_def);
                        }
                    } else {
                        if (ssa_ops[idx].op1_def >= 0) {
                            ssa_vars[ssa_ops[idx].op1_def].alias = zend_jit_var_may_alias(op_array, ssa, EX_VAR_TO_NUM(
                                    opline->op1.var));
                        }
                        if (ssa_ops[idx].op2_def >= 0) {
                            ssa_vars[ssa_ops[idx].op2_def].alias = zend_jit_var_may_alias(op_array, ssa, EX_VAR_TO_NUM(
                                    opline->op2.var));
                        }
                        if (ssa_ops[idx].result_def >= 0) {
                            ssa_vars[ssa_ops[idx].result_def].alias = zend_jit_var_may_alias(op_array, ssa,
                    ...
                    if (opline->opcode == ZEND_RECV_INIT
                        ...
                        if (zend_update_type_info(op_array, tssa, script, (zend_op *) opline, ssa_ops + idx,
                                                  ssa_opcodes, optimization_level) == FAILURE) {
                            // TODO:
                            assert(0);
                        }
                    }
            ...
            while (i < op_array->last_var) {
                ...
                if (i < op_array->num_args) {
                    ...
                    if (ssa_vars[v].alias == NO_ALIAS) {
                        ...
                        ssa_var_info[v].type = MAY_BE_UNDEF | MAY_BE_RC1 | MAY_BE_RCN | MAY_BE_REF | MAY_BE_ANY |
                ...
                if (p->func == NULL || (p - 1)->op != ZEND_JIT_TRACE_VM) {
                    ...
                    switch (opline->opcode) {
                        ...
                        case ZEND_INIT_DYNAMIC_CALL:
                            ...
                            //case ZEND_INIT_USER_CALL:
                            //case ZEND_NEW:
                            frame->used_stack = zend_vm_calc_used_stack(opline->extended_value,
                                                                        (zend_function *) p->func);
                            used_stack += frame->used_stack;
        ...
        while (phi) {
            ...
            if (t & MAY_BE_GUARD) {
                if (tssa->vars[phi->ssa_var].alias != NO_ALIAS) {
                    /* pass */
                } else if (((t0 | t1) & (MAY_BE_ANY | MAY_BE_UNDEF | MAY_BE_REF)) ==
                           (t & (MAY_BE_ANY | MAY_BE_UNDEF | MAY_BE_REF))) {
                    if (!((t0 | t1) & MAY_BE_GUARD)) {
                        ssa_var_info[phi->ssa_var].type = t & ~MAY_BE_GUARD;
                    }
                } else if ((t1 & (MAY_BE_ANY | MAY_BE_UNDEF | MAY_BE_REF)) ==
                           (t & (MAY_BE_ANY | MAY_BE_UNDEF | MAY_BE_REF))) {
                    if (!(t1 & MAY_BE_GUARD)
                    ...
                    if ((t0 & (MAY_BE_ANY | MAY_BE_UNDEF | MAY_BE_REF)) !=
                        (t & (MAY_BE_ANY | MAY_BE_UNDEF | MAY_BE_REF))) {
                        ssa_var_info[phi->sources[0]].type = MAY_BE_GUARD | (t & t0);
                    }
                    if ((t1 & (MAY_BE_ANY | MAY_BE_UNDEF | MAY_BE_REF)) !=
                        (t & (MAY_BE_ANY | MAY_BE_UNDEF | MAY_BE_REF))) {
...
static void
zend_jit_close_var(zend_jit_trace_stack *stack, uint32_t n, int *start, int *end, uint8_t *flags, int line) {
...
static void zend_jit_trace_use_var(int line, int var, int def, int use_chain, int *start, int *end, uint8_t *flags,
                                   const zend_ssa *ssa, const zend_op **ssa_opcodes, const zend_op_array *op_array,
...
static zend_lifetime_interval **
zend_jit_trace_allocate_registers(zend_jit_trace_rec *trace_buffer, zend_ssa *ssa, uint32_t parent_trace,
                                  uint32_t exit_num) {
    ...
    for (;; p++) {
        if (p->op == ZEND_JIT_TRACE_VM) {
            ...
            if (ssa_op->op1_use >= 0
                ...
                if (support_opline) {
                    zend_jit_trace_use_var(idx, ssa_op->op1_use, ssa_op->op1_def, ssa_op->op1_use_chain, start, end,
                                           flags, ssa, ssa_opcodes, op_array, op_array_ssa);
                    if (opline->op1_type != IS_CV) {
            ...
            if (ssa_op->op2_use >= 0
                ...
                if (support_opline) {
                    zend_jit_trace_use_var(idx, ssa_op->op2_use, ssa_op->op2_def, ssa_op->op2_use_chain, start, end,
                                           flags, ssa, ssa_opcodes, op_array, op_array_ssa);
            ...
            if (ssa_op->result_use >= 0
                ...
                if (support_opline) {
                    zend_jit_trace_use_var(idx, ssa_op->result_use, ssa_op->result_def, ssa_op->res_use_chain, start,
                                           end, flags, ssa, ssa_opcodes, op_array, op_array_ssa);
                } else {
            ...
            switch (opline->opcode) {
                ...
                case ZEND_ASSIGN_STATIC_PROP_REF:
                    ...
                    if (ssa_op->op1_use >= 0
                        ...
                        && !zend_ssa_is_no_val_use(opline, ssa_op, ssa_op->op1_use)) {
                        if (support_opline) {
                            zend_jit_trace_use_var(idx, ssa_op->op1_use, ssa_op->op1_def, ssa_op->op1_use_chain, start,
                                                   end, flags, ssa, ssa_opcodes, op_array, op_array_ssa);
                            if (opline->op1_type != IS_CV) {
                                flags[ssa_op->op1_use] |= ZREG_LAST_USE;
                            }
...
static void zend_jit_trace_clenup_stack(zend_jit_trace_stack *stack, const zend_op *opline, const zend_ssa_op *ssa_op,
                                        const zend_ssa *ssa, zend_lifetime_interval **ra) {
...
static void
zend_jit_trace_update_condition_ranges(const zend_op *opline, const zend_ssa_op *ssa_op, const zend_op_array *op_array,
            ...
            while (phi) {
                ...
                if (ival) {
                    if (ival->flags & ZREG_LOAD) {
                        ...
                        if (!zend_jit_load_var(&dasm_state, ssa->var_info[phi->ssa_var].type,
                                               ssa->vars[phi->ssa_var].var, ival->reg)) {
                            goto jit_failure;
                        }
                        ...
                        if (!zend_jit_store_var(&dasm_state, ssa->var_info[phi->ssa_var].type,
                                                ssa->vars[phi->ssa_var].var, ival->reg,
                ...
                switch (opline->opcode) {
                    ...
                    case ZEND_POST_DEC:
                        ...
                        if (opline->result_type != IS_UNUSED) {
                            ...
                            res_use_info = zend_jit_trace_type_to_info(
                                    STACK_MEM_TYPE(stack, EX_VAR_TO_NUM(opline->result.var)))
                                           & (MAY_BE_UNDEF | MAY_BE_NULL | MAY_BE_FALSE | MAY_BE_TRUE | MAY_BE_LONG |
                                              MAY_BE_DOUBLE);
#else
                        ...
                        if (!zend_jit_inc_dec(&dasm_state, opline,
                                              ...
                                              res_addr,
                                              (op1_def_info & (MAY_BE_DOUBLE | MAY_BE_GUARD)) &&
                                              zend_may_overflow(opline, ssa_op, op_array, ssa),
...
//					case ZEND_DIV: // TODO: check for division by zero ???
                        ...
                        if (opline->opcode == ZEND_ADD &&
                            ...
                            if (!zend_jit_math(&dasm_state, opline,
                                               ...
                                               (res_info & (MAY_BE_DOUBLE | MAY_BE_GUARD)) &&
                                               zend_may_overflow(opline, ssa_op, op_array, ssa),
                                               zend_may_throw(opline, ssa_op, op_array, ssa))) {
                                goto jit_failure;
                            }
                    ...
                    case ZEND_INIT_NS_FCALL_BY_NAME:
                        ...
                        if (!zend_jit_init_fcall(&dasm_state, opline,
                                                 op_array_ssa->cfg.map ? op_array_ssa->cfg.map[opline -
                                                                                               op_array->opcodes] : -1,
                                                 op_array, ssa, ssa_op, frame->call_level, p + 1, used_stack < 0)) {
                            goto jit_failure;
                        }
                        goto done;
                    case ZEND_SEND_VAL:
                    case ZEND_SEND_VAL_EX:
                        ...
                        if (frame->call
                            ...
                            && frame->call->func->type == ZEND_USER_FUNCTION) {
                            if (opline->op1_type == IS_CONST) {
                                zend_jit_trace_send_type(opline, frame->call,
                    ...
                    case ZEND_DO_FCALL_BY_NAME:
                    case ZEND_DO_FCALL:
                        if (!zend_jit_do_fcall(&dasm_state, opline, op_array, op_array_ssa, frame->call_level, -1,
                                               p + 1)) {
                            goto jit_failure;
                        }
                        goto done;
                    ...
                    case ZEND_CASE:
                        ...
                        if ((opline->result_type & (IS_SMART_BRANCH_JMPZ | IS_SMART_BRANCH_JMPNZ)) != 0) {
                            ...
                            const zend_op *exit_opline = zend_jit_trace_get_exit_opline(p + 1, opline + 1,
                                                                                        &exit_if_true);
                            uint32_t exit_point;
                    ...
                    case ZEND_TYPE_CHECK:
                        ...
                        if (!zend_jit_type_check(&dasm_state, opline, op1_info, smart_branch_opcode, -1, -1,
                                                 exit_addr)) {
                            goto jit_failure;
                        }
                        goto done;
                    case ZEND_RETURN:
                        ...
                        if (op_array->type == ZEND_EVAL_CODE
                            ...
                            if (!zend_jit_trace_handler(&dasm_state, op_array, opline,
                                                        zend_may_throw(opline, ssa_op, op_array, ssa), p + 1)) {
                                goto jit_failure;
                            }
                        } else {
                            int j;
                            ...
                            for (j = 0; j < op_array->last_var; j++) {
                                ...
                                if (info &
                                    (MAY_BE_STRING | MAY_BE_ARRAY | MAY_BE_OBJECT | MAY_BE_RESOURCE | MAY_BE_REF)) {
                                    if (!left_frame) {
                                        left_frame = 1;
                                    ...
                                    if (info & (MAY_BE_OBJECT | MAY_BE_RESOURCE | MAY_BE_ARRAY_OF_OBJECT |
...
//					case ZEND_FETCH_DIM_UNSET:
                    case ZEND_FETCH_LIST_W:
                        ...
                        if (!zend_jit_fetch_dim(&dasm_state, opline,
                                                ...
                                                (opline->opcode == ZEND_FETCH_DIM_RW
                                                 ...
                                                 || (op2_info &
                                                     (MAY_BE_UNDEF | MAY_BE_RESOURCE | MAY_BE_ARRAY | MAY_BE_OBJECT))
                        ...
                        if (ssa_op->result_def > 0
                            ...
                            && !(op1_info & (MAY_BE_TRUE | MAY_BE_LONG | MAY_BE_DOUBLE | MAY_BE_STRING | MAY_BE_OBJECT |
                                             MAY_BE_RESOURCE | MAY_BE_REF))
                    ...
                    case ZEND_INIT_METHOD_CALL:
                        ...
                        if (!zend_jit_init_method_call(&dasm_state, opline,
                                                       ...
                                                       op1_info, op1_addr, ce, ce_is_instanceof, delayed_fetch_this,
                                                       op1_ce,
                    ...
                    case ZEND_INIT_DYNAMIC_CALL:
                        ...
                        CHECK_OP2_TRACE_TYPE();
                        frame_flags = TRACE_FRAME_MASK_NESTED;
                        if (!zend_jit_init_closure_call(&dasm_state, opline,
                                                        op_array_ssa->cfg.map ? op_array_ssa->cfg.map[opline -
                                                                                                      op_array->opcodes]
                                                                              : -1, op_array, ssa, ssa_op,
                                                        frame->call_level, p + 1, used_stack < 0)) {
                            goto jit_failure;
                        }
                        goto done;
                    case ZEND_INIT_STATIC_METHOD_CALL:
            ...
            if (ssa_op) {
                ...
                if (ssa_op->result_def >= 0) {
                    ...
                    if (type == IS_LONG
                        && zend_inference_propagate_range(op_array, ssa, (zend_op *) opline, (zend_ssa_op *) ssa_op,
                                                          ssa_op->result_def, &tmp)) {
                ...
                switch (opline->opcode) {
                    ...
                    case ZEND_ASSIGN_STATIC_PROP_REF:
                        ...
                        if (ssa_op->op1_def >= 0) {
                            ...
                            if (type == IS_LONG
                                && zend_inference_propagate_range(op_array, ssa, (zend_op *) opline,
                                                                  (zend_ssa_op *) ssa_op, ssa_op->op1_def, &tmp)) {
                                ssa->var_info[ssa_op->op1_def].range.min = tmp.min;
                                ssa->var_info[ssa_op->op1_def].range.max = tmp.max;
                                ssa->var_info[ssa_op->op1_def].range.underflow = 0;
                                ssa->var_info[ssa_op->op1_def].range.overflow = 0;
                        ...
                        while (opline->opcode == ZEND_RECV_INIT) {
                            if (ssa_op->result_def >= 0) {
                                ...
                                if (ra && ra[ssa_op->result_def]) {
                                    SET_STACK_REG_EX(stack, EX_VAR_TO_NUM(opline->result.var),
                                                     ra[ssa_op->result_def]->reg,
                                                     ra[ssa_op->result_def]->flags & ZREG_STORE);
                                }
                            }
                            ssa_op++;
...
static zend_jit_trace_stop
zend_jit_compile_root_trace(zend_jit_trace_rec *trace_buffer, const zend_op *opline, size_t offset) {
    ...
    if ((ZEND_OP_TRACE_INFO(opline, offset)->trace_flags & ZEND_JIT_TRACE_JITED)) {
        ...
        if (handler) {
            ...
            if (t->stack_map_size) {
                zend_jit_trace_stack *shared_stack_map = (zend_jit_trace_stack *) zend_shared_alloc(
    ...
    while (1) {
        if (p->op == ZEND_JIT_TRACE_VM) {
            ...
            fprintf(stderr, "%04d%*c",
                    ...
                    level, ' ');
            zend_dump_op(op_array, NULL, opline, ZEND_DUMP_RC_INFERENCE, tssa,
                         (tssa && tssa->ops) ? tssa->ops + idx : NULL);
            ...
            if (op1_type != IS_UNKNOWN || op2_type != IS_UNKNOWN || op3_type != IS_UNKNOWN) {
                ...
                if (op1_type != IS_UNKNOWN) {
                    ...
                    if ((p + 1)->op == ZEND_JIT_TRACE_OP1_TYPE) {
                        ...
                        const char *type = (op1_type == 0) ? "undef" : zend_get_type_by_const(
                                op1_type & ~(IS_TRACE_REFERENCE | IS_TRACE_INDIRECT | IS_TRACE_PACKED));
                ...
                if (op2_type != IS_UNKNOWN) {
                    ...
                    if ((p + 1)->op == ZEND_JIT_TRACE_OP2_TYPE) {
                        ...
                        const char *type = (op2_type == 0) ? "undef" : zend_get_type_by_const(
                                op2_type & ~(IS_TRACE_REFERENCE | IS_TRACE_INDIRECT));
                ...
                if (op3_type != IS_UNKNOWN) {
                    const char *ref = (op3_type & IS_TRACE_INDIRECT) ?
                                      ...
                                      ((op3_type & IS_TRACE_REFERENCE) ? "&" : "");
                    const char *type = (op3_type == 0) ? "undef" : zend_get_type_by_const(
                            op3_type & ~(IS_TRACE_REFERENCE | IS_TRACE_INDIRECT));
...
int ZEND_FASTCALL
zend_jit_trace_hot_root(zend_execute_data
*execute_data,
const zend_op *opline
)
{
...
if (
ZEND_OP_TRACE_INFO(opline, offset
)->trace_flags & (ZEND_JIT_TRACE_JITED|ZEND_JIT_TRACE_BLACKLISTED)) {
return 0;
}
...
zend_jit_trace_star_desc (ZEND_OP_TRACE_INFO(opline, offset)
...
EXPECTED (ZEND_JIT_TRACE_STOP_DONE(stop))
...
if (
ZEND_JIT_TRACE_STOP_REPEAT(stop)
) {
...
if (
JIT_G(debug)
& (ZEND_JIT_DEBUG_TRACE_STOP|ZEND_JIT_DEBUG_TRACE_ABORT|ZEND_JIT_DEBUG_TRACE_COMPILED|ZEND_JIT_DEBUG_TRACE_BLACKLIST)) {
fprintf(stderr,
"\n");
}
...
static zend_jit_trace_stop
zend_jit_compile_side_trace(zend_jit_trace_rec *trace_buffer, uint32_t parent_num, uint32_t exit_num,
                            uint32_t polymorphism) {
    zend_jit_trace_stop ret;
    const void *handler;
    uint8_t orig_trigger;
    zend_jit_trace_info *t;
...
int ZEND_FASTCALL
zend_jit_trace_hot_side(zend_execute_data
*execute_data,
uint32_t parent_num, uint32_t
exit_num)
{
zend_jit_trace_stop stop;
int ret = 0;
...
if (zend_jit_traces[zend_jit_traces[parent_num].root].child_count >=
...
if (zend_jit_traces[parent_num].polymorphism >=
JIT_G(max_polymorphic_calls)
- 1) {
...
execute_data = EG(current_execute_data);
return
zend_jit_trace_hot_root(execute_data, EX(opline)
);
}
}
...
zend_execute_data *call = (zend_execute_data * )
regs->r[ZREG_RX];
call->
prev_execute_data = EX(call);
EX(call) = call;
}
...
if (
zend_jit_trace_exit_is_bad(trace_num, exit_num
)) {
...
JIT_G(hot_side_exit)
&&
zend_jit_trace_exit_is_hot(trace_num, exit_num
)) {
return
zend_jit_trace_hot_side(execute_data, trace_num, exit_num
);
}

/* Return 1 to call original handler instead of the same JIT-ed trace */
return (orig_opline == t->opline &&
EX(opline)
== orig_opline);
}

static zend_always_inline uint8_t

zend_jit_trace_supported(const zend_op *opline) {
    switch (opline->opcode) {
        case ZEND_CATCH:
        case ZEND_FAST_CALL:
        case ZEND_FAST_RET:
...
static int zend_jit_setup_hot_trace_counters(zend_op_array *op_array) {
    ...
    jit_extension = (zend_jit_op_array_trace_extension *) zend_shared_alloc(
