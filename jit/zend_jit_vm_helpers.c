...
ZEND_SCREWOPCODE_HANDLER_RET ZEND_FASTCALL
zend_jit_leave_nested_func_helper(uint32_t
call_info EXECUTE_DATA_DC
)
{
zend_execute_data *old_execute_data;

if (
UNEXPECTED(call_info
...
zval_ptr_dtor (EX_VAR(old_opline
...
ZEND_SCREWOPCODE_HANDLER_RET ZEND_FASTCALL
zend_jit_leave_top_func_helper(uint32_t
call_info EXECUTE_DATA_DC
)
{
if (
...
ZEND_SCREWOPCODE_HANDLER_RET ZEND_FASTCALL
zend_jit_leave_func_helper(uint32_t
call_info EXECUTE_DATA_DC
)
{
if (call_info & ZEND_CALL_TOP) {
ZEND_SCREWOPCODE_TAIL_CALL_EX(zend_jit_leave_top_func_helper, call_info);
} else {
ZEND_SCREWOPCODE_TAIL_CALL_EX(zend_jit_leave_nested_func_helper, call_info);
}
}

void ZEND_FASTCALL
zend_jit_copy_extra_args_helper(EXECUTE_DATA_D)
{
...
if (
EXPECTED((op_array
->fn_flags & ZEND_ACC_HAS_TYPE_HINTS) == 0)) {
...
zend_bool ZEND_FASTCALL

zend_jit_deprecated_helper(OPLINE_D) {
...
ZEND_SCREWOPCODE_HANDLER_RET ZEND_FASTCALL
zend_jit_profile_helper(ZEND_SCREWOPCODE_HANDLER_ARGS)
{
...
ZEND_SCREWOPCODE_TAIL_CALL(handler);
}

ZEND_SCREWOPCODE_HANDLER_RET ZEND_FASTCALL
zend_jit_func_counter_helper(ZEND_SCREWOPCODE_HANDLER_ARGS)
{
...
if (
UNEXPECTED(*(jit_extension
->counter) <= 0)) {
*(jit_extension->counter) = ZEND_JIT_COUNTER_INIT;
zend_jit_hot_func(execute_data, opline
);
ZEND_SCREWOPCODE_RETURN();
} else {
zend_vm_opcode_handler_t handler = (zend_vm_opcode_handler_t) jit_extension->orig_handlers[opline -
...
ZEND_SCREWOPCODE_HANDLER_RET ZEND_FASTCALL
zend_jit_loop_counter_helper(ZEND_SCREWOPCODE_HANDLER_ARGS)
{
zend_jit_op_array_hot_extension *jit_extension =
        (zend_jit_op_array_hot_extension *) ZEND_FUNC_INFO(&EX(func)->op_array);
#ifndef HAVE_GCC_GLOBAL_REGS
...
zend_jit_get_constant(const zval *key, uint32_t flags) {
    return _zend_quick_get_constant(key, flags, 0);
}

int ZEND_FASTCALL

zend_jit_check_constant(const zval *key) {
    return _zend_quick_get_constant(key, 0, 1);
}

static zend_always_inline ZEND_SCREWOPCODE_HANDLER_RET ZEND_FASTCALL
zend_jit_trace_counter_helper(uint32_t
cost ZEND_SCREWOPCODE_HANDLER_ARGS_DC
)
{
...
if (
UNEXPECTED(*(ZEND_OP_TRACE_INFO(opline, offset)->counter) <= 0)) {
*(ZEND_OP_TRACE_INFO(opline, offset)->counter) = ZEND_JIT_COUNTER_INIT;
if (

UNEXPECTED (zend_jit_trace_hot_root(execute_data, opline)
...
ZEND_SCREWOPCODE_HANDLER_RET ZEND_FASTCALL
zend_jit_func_trace_helper(ZEND_SCREWOPCODE_HANDLER_ARGS)
{
ZEND_SCREWOPCODE_TAIL_CALL_EX(zend_jit_trace_counter_helper,
                              ((ZEND_JIT_COUNTER_INIT + JIT_G(hot_func) - 1) / JIT_G(hot_func)));
}

ZEND_SCREWOPCODE_HANDLER_RET ZEND_FASTCALL
zend_jit_ret_trace_helper(ZEND_SCREWOPCODE_HANDLER_ARGS)
{
ZEND_SCREWOPCODE_TAIL_CALL_EX(zend_jit_trace_counter_helper,
...
ZEND_SCREWOPCODE_HANDLER_RET ZEND_FASTCALL
zend_jit_loop_trace_helper(ZEND_SCREWOPCODE_HANDLER_ARGS)
{
ZEND_SCREWOPCODE_TAIL_CALL_EX(zend_jit_trace_counter_helper,
...
static int
zend_jit_trace_recursive_call_count(const zend_op_array *op_array, const zend_op_array **unrolled_calls, int ret_level,
                                    int level) {
    int i;
    int count = 0;
...
static int
zend_jit_trace_recursive_ret_count(const zend_op_array *op_array, const zend_op_array **unrolled_calls, int ret_level) {
    int i;
    int count = 0;
...
static int
zend_jit_trace_has_recursive_ret(zend_execute_data *ex, const zend_op_array *orig_op_array, const zend_op *orig_opline,
                                 int ret_level) {
...
static int zend_jit_trace_record_fake_init_call_ex(zend_execute_data *call, zend_jit_trace_rec *trace_buffer, int idx,
                                                   uint32_t is_megamorphic, uint32_t *megamorphic, uint32_t level,
                                                   uint32_t init_level, uint32_t *call_level) {
    zend_jit_trace_stop stop
    ZEND_ATTRIBUTE_UNUSED = ZEND_JIT_TRACE_STOP_ERROR;
    ...
    do {
        ...
        if (call->prev_execute_data) {
            idx = zend_jit_trace_record_fake_init_call_ex(call->prev_execute_data, trace_buffer, idx, is_megamorphic,
                                                          megamorphic, level, init_level + 1, call_level);
            if (idx < 0) {
                return idx;
            }
        }
...
static int zend_jit_trace_record_fake_init_call(zend_execute_data *call, zend_jit_trace_rec *trace_buffer, int idx,
    ...
    return zend_jit_trace_record_fake_init_call_ex(call, trace_buffer, idx, is_megamorphic, megamorphic, level, 0,
                                                   &call_level);
}
...
static int zend_jit_trace_subtrace(zend_jit_trace_rec *trace_buffer, int start, int end, uint8_t event,
...
zend_jit_trace_stop ZEND_FASTCALL
zend_jit_trace_execute(zend_execute_data
*ex,
const zend_op *op, zend_jit_trace_rec
*trace_buffer,
uint8_t start, uint32_t
is_megamorphic)
...
#endif
return
ZEND_JIT_TRACE_STOP_EXCEPTION;
}

if (prev_call) {
int ret = zend_jit_trace_record_fake_init_call(prev_call, trace_buffer, idx, is_megamorphic, &megamorphic,
                                               ret_level + level);
if (ret < 0) {
...
#else
rc = handler(ZEND_SCREWOPCODE_HANDLER_ARGS_PASSTHRU);
if (rc != 0) {
if (rc < 0) {
stop = ZEND_JIT_TRACE_STOP_RETURN;
opline = NULL;
...
unrolled_calls[ret_level + level] = &
EX(func)
->
op_array;
level++;
...
&&
zend_jit_trace_has_recursive_ret(execute_data, trace_buffer[0]
.op_array, orig_opline, ret_level)) {
...
&&
zend_jit_may_be_polymorphic_call(opline
- 1)) {
func = NULL;
