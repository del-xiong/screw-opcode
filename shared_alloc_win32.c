...
#define ACCEL_FILEMAP_NAME "ScrewOpcode.SharedMemoryArea"
#define ACCEL_MUTEX_NAME "ScrewOpcode.SharedMemoryMutex"
#define ACCEL_EVENT_SOURCE "ScrewOpcode"
...
static char *create_name_with_username(char *name) {
    ...
    snprintf(newname, sizeof(newname) - 1, "%s@%.32s@%.20s@%.32s", name, accel_uname_id, sapi_module.name,
             zend_system_id);
...
static int zend_shared_alloc_reattach(size_t requested_size, char **error_in) {
    ...
    if (execute_ex_moved ||
        ...
        if (ZCG(accel_directives).file_cache && ZCG(accel_directives).file_cache_fallback) {
            ...
            /* Map only part of SHM to have access screwopcode shared globals */
            ...
            if (mapping_base == NULL) {
                err = GetLastError();
                zend_win_error_message(ACCEL_LOG_FATAL, "Unable to reattach to screwopcode shared globals", err);
                return ALLOC_FAILURE;
            }
        ...
        if (execute_ex_moved) {
            err = ERROR_INVALID_ADDRESS;
            zend_win_error_message(ACCEL_LOG_FATAL,
                                   "Opcode handlers are unusable due to ASLR. Please setup screwopcode.file_cache and screwopcode.file_cache_fallback directives for more convenient Screw Opcode usage",
                                   err);
        } else {
            err = ERROR_INVALID_ADDRESS;
            zend_win_error_message(ACCEL_LOG_FATAL,
                                   "Base address marks unusable memory region. Please setup screwopcode.file_cache and screwopcode.file_cache_fallback directives for more convenient Screw Opcode usage",
                                   err);
        }
        return ALLOC_FAILURE;
    }
...
static int create_segments(size_t requested_size, zend_shared_segment ***shared_segments_p, int *shared_segments_count,
                           char **error_in) {
    int err = 0, ret;
    ...
    */
...
#else
    void *vista_mapping_base_set[] = {(void *) 0x20000000, (void *) 0x21000000, (void *) 0x30000000,
                                      (void *) 0x31000000, (void *) 0x50000000, 0};
    ...
    do {
        memfile = OpenFileMapping(FILE_MAP_READ | FILE_MAP_WRITE | FILE_MAP_EXECUTE, 0,
    ...
    *shared_segments_p = (zend_shared_segment **) calloc(1, sizeof(zend_shared_segment) + sizeof(void *));
    ...
    if (!ZCG(accel_directives).mmap_base || !*ZCG(accel_directives).mmap_base) {
        ...
        if (sscanf(s, "%p", &default_mapping_base_set[0]) != 1) {
            zend_shared_alloc_unlock_win32();
            zend_win_error_message(ACCEL_LOG_FATAL, "Bad mapping address specified in screwopcode.mmap_base", err);
            return ALLOC_FAILURE;
        }
    }

    do {
        shared_segment->p = mapping_base = MapViewOfFileEx(memfile, FILE_MAP_ALL_ACCESS | FILE_MAP_EXECUTE, 0, 0, 0,
                                                           *wanted_mapping_base);
