...
ZEND_BEGIN_ARG_INFO(arginfo_screwopcode_none, 0)
ZEND_END_ARG_INFO()

ZEND_BEGIN_ARG_INFO_EX(arginfo_screwopcode_get_status, 0, 0, 0)
ZEND_ARG_INFO(0, fetch_scripts)
ZEND_END_ARG_INFO()

ZEND_BEGIN_ARG_INFO_EX(arginfo_screwopcode_compile_file, 0, 0, 1)
ZEND_ARG_INFO(0, file)
ZEND_END_ARG_INFO()

ZEND_BEGIN_ARG_INFO_EX(arginfo_screwopcode_invalidate, 0, 0, 1)
ZEND_ARG_INFO(0, script)
ZEND_ARG_INFO(0, force)
ZEND_END_ARG_INFO()

ZEND_BEGIN_ARG_INFO_EX(arginfo_screwopcode_is_script_cached, 0, 0, 1)
ZEND_ARG_INFO(0, script)
ZEND_END_ARG_INFO()

/* User functions */
static ZEND_FUNCTION(screwopcode_reset);
static ZEND_FUNCTION(screwopcode_invalidate);
static ZEND_FUNCTION(screwopcode_is_script_cached);

/* Private functions */
static ZEND_FUNCTION(screwopcode_get_status);
static ZEND_FUNCTION(screwopcode_compile_file);
static ZEND_FUNCTION(screwopcode_get_configuration);

static zend_function_entry accel_functions[] = {
	/* User functions */
	ZEND_FE(screwopcode_reset, arginfo_screwopcode_none)
	...
	/* Private functions */
	ZEND_FE(screwopcode_get_configuration, arginfo_screwopcode_none)
...
static ZEND_INI_MH(OnUpdateMemoryConsumption)
{
	...
	if (memsize < 8)
	{
		...
		if ((ini_entry = zend_hash_str_find_ptr(EG(ini_directives),
												"screwopcode.memory_consumption",
												sizeof("screwopcode.memory_consumption") - 1)) == NULL)
		{
			return FAILURE;
		}
...
static ZEND_INI_MH(OnUpdateMaxAcceleratedFiles)
{
	...
	if (size < MIN_ACCEL_FILES || size > MAX_ACCEL_FILES)
	{
		...
		if (size < MIN_ACCEL_FILES)
		{
			...
			zend_accel_error(ACCEL_LOG_WARNING, "screwopcode.max_accelerated_files is set below the required minimum (%d).\n", MIN_ACCEL_FILES);
		...
		if ((ini_entry = zend_hash_str_find_ptr(EG(ini_directives),
												"screwopcode.max_accelerated_files",
												sizeof("screwopcode.max_accelerated_files") - 1)) == NULL)
		{
			return FAILURE;
		}
...
static ZEND_INI_MH(OnUpdateMaxWastedPercentage)
{
	...
	if (percentage <= 0 || percentage > 50)
	{
		...
		percentage = 5;
		zend_accel_error(ACCEL_LOG_WARNING, "screwopcode.max_wasted_percentage must be set between 1 and 50.\n");
		zend_accel_error(ACCEL_LOG_WARNING, ACCELERATOR_PRODUCT_NAME " will use 5%%.\n");
		if ((ini_entry = zend_hash_str_find_ptr(EG(ini_directives),
												"screwopcode.max_wasted_percentage",
												sizeof("screwopcode.max_wasted_percentage") - 1)) == NULL)
		{
			return FAILURE;
		}
		ini_entry->value = zend_string_init(new_new_value, strlen(new_new_value), 1);
...
static ZEND_INI_MH(OnUpdateFileCache)
{
	if (new_value)
	{
		...
		else
		{
			...
			{
#endif
				zend_accel_error(ACCEL_LOG_WARNING, "screwopcode.file_cache must be a full path of accessable directory.\n");
				new_value = NULL;
			}
		}
	}
...
#endif
STD_PHP_INI_BOOLEAN("screwopcode.inherited_hack", "1", PHP_INI_SYSTEM, OnUpdateBool, accel_directives.inherited_hack, zend_accel_globals, accel_globals)
...
STD_PHP_INI_ENTRY("screwopcode.protect_memory", "0", PHP_INI_SYSTEM, OnUpdateBool, accel_directives.protect_memory, zend_accel_globals, accel_globals)
STD_PHP_INI_ENTRY("screwopcode.save_comments", "1", PHP_INI_SYSTEM, OnUpdateBool, accel_directives.save_comments, zend_accel_globals, accel_globals)
...
STD_PHP_INI_ENTRY("screwopcode.file_cache_only", "0", PHP_INI_SYSTEM, OnUpdateBool, accel_directives.file_cache_only, zend_accel_globals, accel_globals)
STD_PHP_INI_ENTRY("screwopcode.file_cache_consistency_checks", "1", PHP_INI_SYSTEM, OnUpdateBool, accel_directives.file_cache_consistency_checks, zend_accel_globals, accel_globals)
#endif
#if ENABLE_FILE_CACHE_FALLBACK
STD_PHP_INI_ENTRY("screwopcode.file_cache_fallback", "1", PHP_INI_SYSTEM, OnUpdateBool, accel_directives.file_cache_fallback, zend_accel_globals, accel_globals)
#endif
...
static ZEND_FUNCTION(screwopcode_get_status)
{
	...
	/* Trivia */
	add_assoc_bool(return_value, "screwopcode_enabled", ZCG(enabled) && (ZCG(counted) || ZCSG(accelerator_enabled)));
...
static ZEND_FUNCTION(screwopcode_get_configuration)
{
...
#endif
	add_assoc_bool(&directives, "screwopcode.inherited_hack", ZCG(accel_directives).inherited_hack);
	...
	add_assoc_long(&directives, "screwopcode.interned_strings_buffer", ZCG(accel_directives).interned_strings_buffer);
	...
	add_assoc_string(&directives, "screwopcode.preferred_memory_model", STRING_NOT_NULL(ZCG(accel_directives).memory_model));
	add_assoc_string(&directives, "screwopcode.blacklist_filename", STRING_NOT_NULL(ZCG(accel_directives).user_blacklist_filename));
...
#ifndef ZEND_WIN32
	add_assoc_string(&directives, "screwopcode.lockfile_path", STRING_NOT_NULL(ZCG(accel_directives).lockfile_path));
#endif

#ifdef HAVE_OPCACHE_FILE_CACHE
	add_assoc_string(&directives, "screwopcode.file_cache", ZCG(accel_directives).file_cache ? ZCG(accel_directives).file_cache : "");
	add_assoc_bool(&directives, "screwopcode.file_cache_only", ZCG(accel_directives).file_cache_only);
	add_assoc_bool(&directives, "screwopcode.file_cache_consistency_checks", ZCG(accel_directives).file_cache_consistency_checks);
#endif
...
/* {{{ proto void screwopcode_invalidate(string $script [, bool $force = false])
...
/* {{{ proto bool screwopcode_is_script_cached(string $script)
