    ...
    *op_array = persistent_script->script.main_op_array;
    ...
    if (EXPECTED(from_shared_memory)) {
        ...
        if (persistent_script->compiler_halt_offset != 0 &&
            ...
            name = zend_mangle_property_name(haltoff, sizeof(haltoff) - 1, ZSTR_VAL(persistent_script->script.filename),
                                             ZSTR_LEN(persistent_script->script.filename), 0);
            if (!zend_hash_exists(EG(zend_constants), name)) {
                zend_register_long_constant(ZSTR_VAL(name), ZSTR_LEN(name), persistent_script->compiler_halt_offset,
                                            CONST_CS, 0);
            }
...
unsigned int zend_accel_script_checksum(zend_persistent_script *persistent_script) {
    ...
    size_t persistent_script_check_block_size =
            ((char *) &(persistent_script->dynamic_members)) - (char *) persistent_script;
