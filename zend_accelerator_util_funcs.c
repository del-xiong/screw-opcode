...
failure:
	...
	if (function2->type == ZEND_USER_FUNCTION && function2->op_array.last > 0)
	{
...
#define ADLER32_DO1(buf) \
	{                    \
		s1 += *(buf);    \
		s2 += s1;        \
	}
#define ADLER32_DO2(buf, i) \
	ADLER32_DO1(buf + i);   \
	ADLER32_DO1(buf + i + 1);
#define ADLER32_DO4(buf, i) \
	ADLER32_DO2(buf, i);    \
	ADLER32_DO2(buf, i + 2);
#define ADLER32_DO8(buf, i) \
	ADLER32_DO4(buf, i);    \
	ADLER32_DO4(buf, i + 4);
#define ADLER32_DO16(buf) \
	ADLER32_DO8(buf, 0);  \
	ADLER32_DO8(buf, 8);
