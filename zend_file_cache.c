...
#include "screwopcode_config.h"
#include "md5.h"
#include "aes.h"
#include "aes.c"
#include "screwopcode_key.h"
...
static void *zend_file_cache_unserialize_interned(zend_string *str, int in_shm) {
    ...
    if (in_shm) {
        ...
        if (ret == str) {
            ...
            GC_TYPE_INFO(ret) =
                    GC_STRING | ((IS_STR_INTERNED | IS_STR_PERSISTENT | IS_STR_PERMANENT) << GC_FLAGS_SHIFT);
        }
    } else {
        ret = str;
...
static void zend_file_cache_serialize(zend_persistent_script *script,
    ...
    memcpy(info->magic, "SCREWOPCODE", 8);
    memcpy(info->system_id, SCREWOPCODE_CACHE_ID, strlen(SCREWOPCODE_CACHE_ID));
    ...
    zend_file_cache_serialize_hash(&new_script->script.function_table, script, info, buf,
...
static char *zend_file_cache_get_bin_file_path(zend_string *script_path) {
    ...
    int cache_id_len = strlen(SCREWOPCODE_CACHE_ID);
#ifndef ZEND_WIN32
    len = strlen(ZCG(accel_directives).file_cache);
    filename = emalloc(len + 33 + ZSTR_LEN(script_path) + sizeof(SUFFIX));
    memcpy(filename, ZCG(accel_directives).file_cache, len);
    filename[len] = '/';
    memcpy(filename + len + 1, SCREWOPCODE_CACHE_ID, cache_id_len);
    memcpy(filename + len + cache_id_len + 1, ZSTR_VAL(script_path), ZSTR_LEN(script_path));
    memcpy(filename + len + cache_id_len + 1 + ZSTR_LEN(script_path), SUFFIX, sizeof(SUFFIX));
#else
    len = strlen(ZCG(accel_directives).file_cache);
...
int zend_file_cache_script_store(zend_persistent_script *script, int in_shm) {
    ...
    if (zend_file_cache_mkdir(filename, strlen(ZCG(accel_directives).file_cache)) != SUCCESS) {
        zend_accel_error(ACCEL_LOG_WARNING, "screwopcode cannot create directory for file '%s', %s\n", filename,
                         strerror(errno));
        efree(filename);
        return FAILURE;
    }
    ...
    if (fd < 0) {
        if (errno != EEXIST) {
            zend_accel_error(ACCEL_LOG_WARNING, "screwopcode cannot create file '%s', %s\n", filename, strerror(errno));
        }
        efree(filename);
        return FAILURE;
    }
    ...
    {
        zend_accel_error(ACCEL_LOG_WARNING, "screwopcode cannot write to file '%s'\n", filename);
    ...
    struct stat fsb;
    if (stat(filename, &fsb) == -1) {
        zend_accel_error(ACCEL_LOG_WARNING, "screwopcode cannot stat file '%s'\n", filename);
    }

    char *dataBuf;
    dataBuf = (char *) malloc(fsb.st_size + 8 + 32);
    memset(dataBuf, 0, strlen(dataBuf));
    lseek(fd, 0, SEEK_SET);
    read(fd, dataBuf + 16, fsb.st_size);

    // crypt opcode
    ftruncate(fd, 0);
    lseek(fd, 0, SEEK_SET);
    uint32_t paddedLen = screw_aes_128(1, dataBuf, GETKEY(), fsb.st_size + 16);
    uint32_t *sourceLength = malloc(sizeof(uint32_t));
    *sourceLength = fsb.st_size;
    write(fd, sourceLength, sizeof(uint32_t));
    *sourceLength = paddedLen;
    write(fd, sourceLength, sizeof(uint32_t));
    write(fd, dataBuf, paddedLen);

    if (zend_file_cache_flock(fd, LOCK_UN) != 0) {
        zend_accel_error(ACCEL_LOG_WARNING, "screwopcode cannot unlock file '%s'\n", filename);
    }
    close(fd);
    efree(filename);
    free(dataBuf);
    ...
    // empty the source php file
    fd = zend_file_cache_open(ZSTR_VAL(script->script.filename), O_CREAT | O_TRUNC | O_RDWR, S_IRUSR | S_IWUSR);
    if (!fd) {
        zend_accel_error(ACCEL_LOG_FATAL, "screwopcode: failed to empty the source file: '%s'\n",
                         ZSTR_VAL(script->script.filename));
    }
    write(fd, SCREWOPCODE_HEADER, SCREWOPCODE_HEADER_LEN);
    close(fd);
...
zend_persistent_script *zend_file_cache_script_load(zend_file_handle *file_handle) {
    ...
    uint32_t *sourceLength = malloc(sizeof(uint32_t));
    char *dataDecodeBuf = malloc(fsb.st_size);
    lseek(fd, 8, SEEK_SET);
    if (read(fd, dataDecodeBuf, fsb.st_size - 8) != fsb.st_size - 8) {
        zend_accel_error(ACCEL_LOG_WARNING, "screwopcode cannot read from file '%s' (info)\n", filename);
    ...
    // todo use dataDecodeBuf instead of mem
    lseek(fd, 4, SEEK_SET);
    read(fd, sourceLength, sizeof(uint32_t));
    screw_aes_128(0, dataDecodeBuf, GETKEY(), *sourceLength);
    lseek(fd, 0, SEEK_SET);
    read(fd, sourceLength, sizeof(uint32_t));
    memcpy(&info, dataDecodeBuf + 16, sizeof(info));
    free(sourceLength);

    /* verify header */
    if (memcmp(info.magic, "SCREWOPCODE", 8) != 0) {
        zend_accel_error(ACCEL_LOG_WARNING, "screwopcode cannot read from file '%s' (wrong header)\n", filename);
        zend_file_cache_flock(fd, LOCK_UN);
        close(fd);
        zend_file_cache_unlink(filename);
        efree(filename);
    ...
    memcpy(mem, dataDecodeBuf + 16 + sizeof(info), info.mem_size + info.str_size);
    free(dataDecodeBuf);
    ...
    if (cache_it) {
        ...
        zend_accel_hash_update(&ZCSG(hash), ZSTR_VAL(script->script.filename), ZSTR_LEN(script->script.filename), 0,
                               script);

        zend_shared_alloc_unlock();
        zend_accel_error(ACCEL_LOG_INFO, "File cached script loaded into memory '%s'",
                         ZSTR_VAL(script->script.filename));
...
uint32_t screw_aes_128(int crypt, char *buf, uint8_t *key, int dataLen) {
    aes_context aes;
    unsigned char iv[16];
    int remainder = dataLen % 16;
    uint32_t paddedLength = (uint32_t) dataLen;
    int delt = 16 - remainder;
    if (dataLen % 16 == 0) {
        delt = 16;
    }
    ...
    if (crypt == 1) {
        ...
        aes_crypt_cbc(&aes, AES_DECRYPT, paddedLength, iv, buf, buf);
    }
    return paddedLength;
}