...
static void zend_hash_persist(HashTable *ht) {
    ...
    if (HT_FLAGS(ht) & HASH_FLAG_PACKED) {
        ...
        ZCG(mem) = (void *) ((char *) ZCG(mem) +
                             ZEND_ALIGNED_SIZE((hash_size * sizeof(uint32_t)) + (ht->nNumUsed * sizeof(Bucket))));
        HT_HASH_RESET(ht);
...
static zend_ast *zend_persist_ast(zend_ast *ast) {
    ...
    if (ast->kind == ZEND_AST_ZVAL || ast->kind == ZEND_AST_CONSTANT) {
        ...
        zend_ast_list *copy = zend_shared_memdup(ast,
                                                 sizeof(zend_ast_list) - sizeof(zend_ast * ) +
                                                 sizeof(zend_ast * ) * list->children);
...
static void zend_persist_op_array_ex(zend_op_array *op_array, zend_persistent_script *main_persistent_script) {
    ...
    if (main_persistent_script) {
        ...
        if ((offset = zend_get_constant_str("__COMPILER_HALT_OFFSET__", sizeof("__COMPILER_HALT_OFFSET__") - 1)) !=
            NULL) {
    ...
    if (op_array->scope) {
        ...
        if (persist_ptr) {
            ...
            ZCG(mem) = (void *) ((char *) ZCG(mem) +
                                 ZEND_ALIGNED_SIZE(zend_extensions_op_array_persist(op_array, ZCG(mem))));
#ifdef HAVE_JIT
    ...
    if (op_array->static_variables) {
        ...
        GC_TYPE_INFO(op_array->static_variables) =
    ...
    if (op_array->live_range) {
        op_array->live_range = zend_shared_memdup_put_free(op_array->live_range,
    ...
    if (op_array->try_catch_array) {
        op_array->try_catch_array = zend_shared_memdup_put_free(op_array->try_catch_array,
                                                                sizeof(zend_try_catch_element) *
...
static void zend_persist_class_entry(zval *zv) {
    ...
    if (ce->type == ZEND_USER_CLASS) {
        ...
        if (ce->default_properties_table) {
            ...
            ce->default_properties_table = zend_shared_memdup_free(ce->default_properties_table,
                                                                   sizeof(zval) * ce->default_properties_count);
        ...
        if (ce->default_static_members_table) {
            int i;
            ce->default_static_members_table = zend_shared_memdup_free(ce->default_static_members_table,
                                                                       sizeof(zval) * ce->default_static_members_count);
        ...
        if (ce->num_interfaces && !(ce->ce_flags & ZEND_ACC_LINKED)) {
            ...
            ce->interface_names = zend_shared_memdup_free(ce->interface_names,
                                                          sizeof(zend_class_name) * ce->num_interfaces);
        }
        ...
        if (ce->num_traits) {
            ...
            if (ce->trait_precedences) {
                ...
                while (ce->trait_precedences[i]) {
                    ...
                    ce->trait_precedences[i] = zend_shared_memdup_free(ce->trait_precedences[i],
                                                                       sizeof(zend_trait_precedence) +
                                                                       (ce->trait_precedences[i]->num_excludes - 1) *
                                                                       sizeof(zend_string * ));
                    i++;
                }
    ...
    if (ce->ce_flags & ZEND_ACC_LINKED) {
        ...
        if (ce->iterator_funcs_ptr) {
            ...
            if (zend_class_implements_interface(ce, zend_ce_aggregate)) {
                ce->iterator_funcs_ptr->zf_new_iterator = zend_hash_str_find_ptr(&ce->function_table, "getiterator",
                                                                                 sizeof("getiterator") - 1);
            }
            if (zend_class_implements_interface(ce, zend_ce_iterator)) {
                ce->iterator_funcs_ptr->zf_rewind = zend_hash_str_find_ptr(&ce->function_table, "rewind",
                                                                           sizeof("rewind") - 1);
                ce->iterator_funcs_ptr->zf_valid = zend_hash_str_find_ptr(&ce->function_table, "valid",
                                                                          sizeof("valid") - 1);
                ce->iterator_funcs_ptr->zf_key = zend_hash_str_find_ptr(&ce->function_table, "key", sizeof("key") - 1);
                ce->iterator_funcs_ptr->zf_current = zend_hash_str_find_ptr(&ce->function_table, "current",
                                                                            sizeof("current") - 1);
                ce->iterator_funcs_ptr->zf_next = zend_hash_str_find_ptr(&ce->function_table, "next",
                                                                         sizeof("next") - 1);
            }
        }
    }
