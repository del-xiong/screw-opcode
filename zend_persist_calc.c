...
static void zend_hash_persist_calc(HashTable *ht) {
    ...
    if (!(HT_FLAGS(ht) & HASH_FLAG_PACKED) && ht->nNumUsed > HT_MIN_SIZE &&
        ht->nNumUsed < (uint32_t)(-(int32_t) ht->nTableMask) / 4) {
        /* compact table */
        uint32_t hash_size;
            ...
            if (ce->trait_precedences) {
                ...
                while (ce->trait_precedences[i]) {
                    ...
                    ADD_SIZE(sizeof(zend_trait_precedence) +
                             (ce->trait_precedences[i]->num_excludes - 1) * sizeof(zend_string * ));
                    i++;
                }
...
uint32_t
zend_accel_script_persist_calc(zend_persistent_script *new_persistent_script, const char *key, unsigned int key_length,
                               int for_shm) {
    Bucket *p;
    ...
    if (new_persistent_script->script.class_table.nNumUsed !=
        new_persistent_script->script.class_table.nNumOfElements) {
    ...
    if (new_persistent_script->script.function_table.nNumUsed !=
        new_persistent_script->script.function_table.nNumOfElements) {
