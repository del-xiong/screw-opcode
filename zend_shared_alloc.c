...
static void no_memory_bailout(size_t allocate_size, char *error) {
    zend_accel_error(ACCEL_LOG_FATAL, "Unable to allocate shared memory segment of %zu bytes: %s: %s (%d)",
                     allocate_size, error ? error : "unknown", strerror(errno), errno);
}
...
static int zend_shared_alloc_try(const zend_shared_memory_handler_entry *he, size_t requested_size,
                                 zend_shared_segment ***shared_segments_p, int *shared_segments_count,
                                 char **error_in) {
    int res;
...
int zend_shared_alloc_startup(size_t requested_size, size_t reserved_size) {
    ...
    if (ZCG(accel_directives).memory_model && ZCG(accel_directives).memory_model[0]) {
        ...
        for (he = handler_table; he->name; he++) {
            if (strcmp(model, he->name) == 0) {
                res = zend_shared_alloc_try(he, requested_size, &ZSMMG(shared_segments), &ZSMMG(shared_segments_count),
                                            &error_in);
                if (res) {
                    /* this model works! */
                }
    ...
    copy_shared_segments(tmp_shared_segments, ZSMMG(shared_segments)[0], ZSMMG(shared_segments_count),
...
_zend_shared_memdup(void *source, size_t size, zend_bool arena, zend_bool get_xlat, zend_bool set_xlat,
                    zend_bool free_source) {
    void *old_p, *retval;
    zend_ulong key;
